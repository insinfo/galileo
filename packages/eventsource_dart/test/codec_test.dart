...
import "package:eventsource_dart/src/decoder.dart";
import "package:eventsource_dart/src/encoder.dart";
import "package:eventsource_dart/src/event.dart";

Map<Event, String> _VECTORS = {
  new Event(id: "1", event: "Add", data: "This is a test"): "id: 1\nevent: Add\ndata: This is a test\n\n",
...
void main() {
  ...
  group("decoder", () {
    test("vectors", () async {
      for (Event event in _VECTORS.keys) {
        var encoded = _VECTORS[event];
        var stream = Stream.fromIterable([encoded]).transform(Utf8Encoder()).transform(EventSourceDecoder());
