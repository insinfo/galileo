library eventsource_dart.io_server;
...
Function createIoHandler(EventSourcePublisher publisher, {String channel: "", bool gzip: false}) {
  void ioHandler(io.HttpRequest request) {
    ...
    bool useGzip = gzip && (request.headers.value(io.HttpHeaders.ACCEPT_ENCODING) ?? "").contains("gzip");
    ...
    var encodedSink = new EventSourceEncoder(compressed: useGzip).startChunkedConversion(response);
    ...
    publisher.newSubscription(
        onEvent: onEvent, onClose: onClose, channel: channel, lastEventId: request.headers.value("Last-Event-ID"));
  }
