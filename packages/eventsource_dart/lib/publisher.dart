library eventsource_dart.server;
...
class EventSourcePublisher extends Sink<Event> {
  ...
  Sink<Event> channel(String channel) =>
      new ProxySink(onAdd: (e) => add(e, channels: [channel]), onClose: () => close(channels: [channel]));
  ...
  void add(Event event, {Iterable<String> channels: const [""]}) {
    for (String channel in channels) {
      ...
      _logFiner("Sending event on channel $channel to ${subs.length} subscribers.");
      for (var sub in subs) {
        sub.add(event);
      }
    }
  ...
  void newSubscription({Function onEvent, Function onClose, String channel, String lastEventId}) {
