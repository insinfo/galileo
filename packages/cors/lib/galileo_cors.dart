...
Future<bool> Function(RequestContext, ResponseContext) cors([CorsOptions options]) {
  options ??= CorsOptions();
  ...
  return (req, res) async {
    ...
    if (req.method == 'OPTIONS' && options.allowedHeaders.isNotEmpty) {
      ...
      res.headers['access-control-allow-headers'] = req.headers.value('access-control-request-headers');
    }
    ...
    if (options.origin == false || options.origin == '*') {
      ...
      bool isAllowed = _isOriginAllowed(req.headers.value('origin'), options.origin);

      res.headers['access-control-allow-origin'] = isAllowed ? req.headers.value('origin') : false.toString();
