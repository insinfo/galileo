...
Future<void> insertRows(MySqlConnection conn) async {
  ...
  List<StreamedResults> r1 = await (await conn.preparedWithAll("INSERT INTO people (name, age) VALUES (?, ?)", [
    ["Dave", 15],
    ["John", 16],
    ["Mavis", 93],
  ]))
  ...
  List<StreamedResults> r2 =
      await (await conn.preparedWithAll("INSERT INTO pets (name, species, owner_id) VALUES (?, ?, ?)", [
    ["Rover", "Dog", 1],
    ["Daisy", "Cow", 2],
    ["Spot", "Dog", 2]
  ]))
...
Future<void> readData(MySqlConnection conn) async {
  StreamedResults result = await conn.execute('SELECT p.id, p.name, p.age, t.name AS pet, t.species '
      'FROM people p '
