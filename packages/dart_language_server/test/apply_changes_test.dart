...
import 'package:galileo_dart_language_server/src/apply_change.dart';
import 'package:galileo_dart_language_server/src/protocol/language_server/messages.dart';

void main() {
  void testApplyChange(String oldText, String newText, int startLine, int startCharacter, int endLine, int endCharacter,
      int rangeLength, String text) {
    var original = oldText.split('\n').map((l) => l.length).toList();
    var expected = newText.split('\n').map((l) => l.length).toList();
    expect(
        applyChange(original, _change(startLine, startCharacter, endLine, endCharacter, rangeLength, text)), expected);
  }
...
TextDocumentContentChangeEvent _change(
        int startLine, int startCharacter, int endLine, int endCharacter, int rangeLength, String text) =>
