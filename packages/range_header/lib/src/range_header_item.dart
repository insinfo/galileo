...
class RangeHeaderItem implements Comparable<RangeHeaderItem> {
  ...
  RangeHeaderItem consolidate(RangeHeaderItem other) {
    if (!(other.overlaps(this))) throw new ArgumentError('The two ranges do not overlap.');
  ...
  @override
  bool operator ==(other) => other is RangeHeaderItem && other.start == start && other.end == end;
  ...
  String toContentRange([int totalSize]) {
    ...
    if (end == -1) {
      if (totalSize == null) {
        throw new UnsupportedError('If the end of this range is unknown, `totalSize` must not be null.');
      } else {
        // if (end == totalSize - 1) {
