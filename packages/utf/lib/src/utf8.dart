...
String decodeUtf8(List<int> bytes,
    [int offset = 0, int length, int replacementCodepoint = UNICODE_REPLACEMENT_CHARACTER_CODEPOINT]) {
  return new String.fromCharCodes((new Utf8Decoder(bytes, offset, length, replacementCodepoint)).decodeRest());
}
...
int _addToEncoding(int offset, int bytes, int value, List<int> buffer) {
  while (bytes > 0) {
    buffer[offset + bytes] = _UTF8_SUBSEQUENT_BYTE_BASE | (value & _UTF8_LO_SIX_BIT_MASK);
    value = value >> 6;
    bytes--;
  }
  return value;
...
List<int> codepointsToUtf8(List<int> codepoints, [int offset = 0, int length]) {
  ...
  for (int value in source) {
    if (value < 0 || value > UNICODE_VALID_RANGE_MAX) {
      ...
      encoded[insertAt] =
          _UTF8_FIRST_BYTE_OF_TWO_BASE | (_UTF8_FIRST_BYTE_OF_TWO_MASK & _addToEncoding(insertAt, 1, value, encoded));
      insertAt += 2;
    } else if (value <= _UTF8_THREE_BYTE_MAX) {
      encoded[insertAt] = _UTF8_FIRST_BYTE_OF_THREE_BASE |
          (_UTF8_FIRST_BYTE_OF_THREE_MASK & _addToEncoding(insertAt, 2, value, encoded));
      insertAt += 3;
    } else if (value <= UNICODE_VALID_RANGE_MAX) {
      encoded[insertAt] =
          _UTF8_FIRST_BYTE_OF_FOUR_BASE | (_UTF8_FIRST_BYTE_OF_FOUR_MASK & _addToEncoding(insertAt, 3, value, encoded));
      insertAt += 4;
    }
  }
  return encoded;
...
List<int> utf8ToCodepoints(List<int> utf8EncodedBytes,
    [int offset = 0, int length, int replacementCodepoint = UNICODE_REPLACEMENT_CHARACTER_CODEPOINT]) {
  return new Utf8Decoder(utf8EncodedBytes, offset, length, replacementCodepoint).decodeRest();
}
...
class IterableUtf8Decoder extends IterableBase<int> {
  ...
  IterableUtf8Decoder(this.bytes,
      [this.offset = 0, this.length, this.replacementCodepoint = UNICODE_REPLACEMENT_CHARACTER_CODEPOINT]);
...
class Utf8Decoder implements Iterator<int> {
  ...
  Utf8Decoder(List<int> utf8EncodedBytes,
      [int offset = 0, int length, this.replacementCodepoint = UNICODE_REPLACEMENT_CHARACTER_CODEPOINT])
      : utf8EncodedBytesIterator = (new ListRange(utf8EncodedBytes, offset, length)).iterator;
  ...
  /** Decode the remaininder of the characters in this decoder
    ...
    */
  List<int> decodeRest() {
    List<int> codepoints = List<int>.filled(utf8EncodedBytesIterator.remaining, null);
    int i = 0;
    while (moveNext()) {
    ...
    if (i == codepoints.length) {
      ...
      List<int> truncCodepoints = List<int>.filled(i, null);
  ...
  bool moveNext() {
    ...
    if (value < 0) {
      if (replacementCodepoint != null) {
        ...
        throw new ArgumentError("Invalid UTF8 at ${utf8EncodedBytesIterator.position}");
      }
    } else if (value <= _UTF8_ONE_BYTE_MAX) {
      _current = value;
      return true;
    ...
    while (j < additionalBytes && utf8EncodedBytesIterator.moveNext()) {
      ...
      if (nextValue > _UTF8_ONE_BYTE_MAX && nextValue < _UTF8_FIRST_BYTE_OF_TWO_BASE) {
    ...
    bool validSequence =
        (j == additionalBytes && (value < UNICODE_UTF16_RESERVED_LO || value > UNICODE_UTF16_RESERVED_HI));
    ...
    if (validSequence && nonOverlong && inRange) {
      ...
      throw new ArgumentError("Invalid UTF8 at ${utf8EncodedBytesIterator.position - j}");
    }
  }
}
