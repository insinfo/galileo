...
IterableUtf16Decoder decodeUtf16AsIterable(List<int> bytes,
  ...
  return new IterableUtf16Decoder._(
      () => new Utf16BytesToCodeUnitsDecoder(bytes, offset, length, replacementCodepoint), replacementCodepoint);
}
...
IterableUtf16Decoder decodeUtf16beAsIterable(List<int> bytes,
  ...
  return new IterableUtf16Decoder._(
      () => new Utf16beBytesToCodeUnitsDecoder(bytes, offset, length, stripBom, replacementCodepoint),
...
IterableUtf16Decoder decodeUtf16leAsIterable(List<int> bytes,
    ...
    int replacementCodepoint = UNICODE_REPLACEMENT_CHARACTER_CODEPOINT]) {
  return new IterableUtf16Decoder._(
      () => new Utf16leBytesToCodeUnitsDecoder(bytes, offset, length, stripBom, replacementCodepoint),
      replacementCodepoint);
}
...
String decodeUtf16(List<int> bytes,
    [int offset = 0, int length, int replacementCodepoint = UNICODE_REPLACEMENT_CHARACTER_CODEPOINT]) {
  Utf16BytesToCodeUnitsDecoder decoder = new Utf16BytesToCodeUnitsDecoder(bytes, offset, length, replacementCodepoint);
  List<int> codeunits = decoder.decodeRest();
  return new String.fromCharCodes(utf16CodeUnitsToCodepoints(codeunits, 0, null, replacementCodepoint));
}
...
String decodeUtf16be(List<int> bytes,
    ...
    int replacementCodepoint = UNICODE_REPLACEMENT_CHARACTER_CODEPOINT]) {
  List<int> codeunits =
      (new Utf16beBytesToCodeUnitsDecoder(bytes, offset, length, stripBom, replacementCodepoint)).decodeRest();
  return new String.fromCharCodes(utf16CodeUnitsToCodepoints(codeunits, 0, null, replacementCodepoint));
}
...
String decodeUtf16le(List<int> bytes,
    ...
    int replacementCodepoint = UNICODE_REPLACEMENT_CHARACTER_CODEPOINT]) {
  List<int> codeunits =
      (new Utf16leBytesToCodeUnitsDecoder(bytes, offset, length, stripBom, replacementCodepoint)).decodeRest();
  return new String.fromCharCodes(utf16CodeUnitsToCodepoints(codeunits, 0, null, replacementCodepoint));
}
...
List<int> encodeUtf16be(String str, [bool writeBOM = false]) {
  ...
  List<int> encoding = new List<int>(2 * utf16CodeUnits.length + (writeBOM ? 2 : 0));
  int i = 0;
  if (writeBOM) {
...
class IterableUtf16Decoder extends IterableBase<int> {
  ...
  Utf16CodeUnitDecoder get iterator =>
      new Utf16CodeUnitDecoder.fromListRangeIterator(codeunitsProvider(), replacementCodepoint);
}
...
abstract class Utf16BytesToCodeUnitsDecoder implements ListRangeIterator {
  ...
  factory Utf16BytesToCodeUnitsDecoder(List<int> utf16EncodedBytes,
    ...
    if (hasUtf16beBom(utf16EncodedBytes, offset, length)) {
      return new Utf16beBytesToCodeUnitsDecoder(utf16EncodedBytes, offset + 2, length - 2, false, replacementCodepoint);
    } else if (hasUtf16leBom(utf16EncodedBytes, offset, length)) {
      return new Utf16leBytesToCodeUnitsDecoder(utf16EncodedBytes, offset + 2, length - 2, false, replacementCodepoint);
    } else {
      return new Utf16beBytesToCodeUnitsDecoder(utf16EncodedBytes, offset, length, false, replacementCodepoint);
    }
  }
  ...
  bool moveNext() {
    ...
    if (remaining == 1) {
      ...
      if (replacementCodepoint != null) {
        ...
        throw new ArgumentError("Invalid UTF16 at ${utf16EncodedBytesIterator.position}");
      }
    }
    _current = decode();
    return true;
...
class Utf16beBytesToCodeUnitsDecoder extends Utf16BytesToCodeUnitsDecoder {
  Utf16beBytesToCodeUnitsDecoder(List<int> utf16EncodedBytes,
      ...
      : super._fromListRangeIterator(
            (new ListRange(utf16EncodedBytes, offset, length)).iterator, replacementCodepoint) {
