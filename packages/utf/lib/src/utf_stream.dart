...
abstract class _StringDecoder extends StreamTransformerBase<List<int>, String> implements EventSink<List<int>> {
  List<int> _carry;
  List<int> _buffer;
  ...
  Stream<String> bind(Stream<List<int>> stream) {
    return new Stream<String>.eventTransformed(stream, (EventSink<String> sink) {
      if (_outSink != null) {
  ...
  void close() {
    if (_carry != null) {
      if (_replacementChar != null) {
        _outSink.add(new String.fromCharCodes(new List.filled(_carry.length, _replacementChar)));
      } else {
...
class Utf8DecoderTransformer extends _StringDecoder {
  Utf8DecoderTransformer([int replacementChar = UNICODE_REPLACEMENT_CHARACTER_CODEPOINT]) : super(replacementChar);
...
abstract class _StringEncoder extends StreamTransformerBase<String, List<int>> implements EventSink<String> {
  EventSink<List<int>> _outSink;

  Stream<List<int>> bind(Stream<String> stream) {
    return new Stream<List<int>>.eventTransformed(stream, (EventSink<List<int>> sink) {
      if (_outSink != null) {
