...
IterableUtf32Decoder decodeUtf32AsIterable(List<int> bytes,
  ...
  return new IterableUtf32Decoder._(() => new Utf32BytesDecoder(bytes, offset, length, replacementCodepoint));
}
...
IterableUtf32Decoder decodeUtf32beAsIterable(List<int> bytes,
  ...
  return new IterableUtf32Decoder._(
      () => new Utf32beBytesDecoder(bytes, offset, length, stripBom, replacementCodepoint));
}
...
IterableUtf32Decoder decodeUtf32leAsIterable(List<int> bytes,
    ...
    int replacementCodepoint = UNICODE_REPLACEMENT_CHARACTER_CODEPOINT]) {
  return new IterableUtf32Decoder._(
      () => new Utf32leBytesDecoder(bytes, offset, length, stripBom, replacementCodepoint));
}
...
String decodeUtf32(List<int> bytes,
    [int offset = 0, int length, int replacementCodepoint = UNICODE_REPLACEMENT_CHARACTER_CODEPOINT]) {
  return new String.fromCharCodes((new Utf32BytesDecoder(bytes, offset, length, replacementCodepoint)).decodeRest());
}
...
String decodeUtf32be(List<int> bytes,
    ...
    new String.fromCharCodes(
        (new Utf32beBytesDecoder(bytes, offset, length, stripBom, replacementCodepoint)).decodeRest());
...
String decodeUtf32le(List<int> bytes,
        ...
        int replacementCodepoint = UNICODE_REPLACEMENT_CHARACTER_CODEPOINT]) =>
    new String.fromCharCodes(
        (new Utf32leBytesDecoder(bytes, offset, length, stripBom, replacementCodepoint)).decodeRest());
...
List<int> encodeUtf32be(String str, [bool writeBOM = false]) {
  ...
  List<int> encoding = new List<int>(4 * utf32CodeUnits.length + (writeBOM ? 4 : 0));
  int i = 0;
  if (writeBOM) {
    encoding[i++] = 0;
    encoding[i++] = 0;
...
abstract class Utf32BytesDecoder implements ListRangeIterator {
  ...
  Utf32BytesDecoder._fromListRangeIterator(this.utf32EncodedBytesIterator, this.replacementCodepoint);
  ...
  factory Utf32BytesDecoder(List<int> utf32EncodedBytes,
    ...
    if (hasUtf32beBom(utf32EncodedBytes, offset, length)) {
      return new Utf32beBytesDecoder(utf32EncodedBytes, offset + 4, length - 4, false, replacementCodepoint);
    } else if (hasUtf32leBom(utf32EncodedBytes, offset, length)) {
      return new Utf32leBytesDecoder(utf32EncodedBytes, offset + 4, length - 4, false, replacementCodepoint);
    } else {
      return new Utf32beBytesDecoder(utf32EncodedBytes, offset, length, false, replacementCodepoint);
    }
  }
  ...
  bool moveNext() {
    ...
    if (remaining < 4) {
      ...
      if (replacementCodepoint != null) {
        ...
        throw new ArgumentError("Invalid UTF32 at ${utf32EncodedBytesIterator.position}");
      }
    }
...
class Utf32beBytesDecoder extends Utf32BytesDecoder {
  Utf32beBytesDecoder(List<int> utf32EncodedBytes,
      ...
      : super._fromListRangeIterator(
            (new ListRange(utf32EncodedBytes, offset, length)).iterator, replacementCodepoint) {
...
bool _validCodepoint(int codepoint) {
  return (codepoint >= 0 && codepoint < UNICODE_UTF16_RESERVED_LO) ||
      (codepoint > UNICODE_UTF16_RESERVED_HI && codepoint < UNICODE_VALID_RANGE_MAX);
}
