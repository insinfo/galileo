...
library galileo_utf.utf82_test;
...
const String testHebrewPhrase = "דג סקרן שט בים מאוכזב ולפתע מצא לו חברה איך הקליטה";
...
void testEncodeToUtf8() {
  expect.listEquals(testEnglishUtf8, encodeUtf8(testEnglishPhrase), "english to utf8");

  expect.listEquals(testDanishUtf8, encodeUtf8(testDanishPhrase), "encode danish to utf8");

  expect.listEquals(testHebrewUtf8, encodeUtf8(testHebrewPhrase), "Hebrew to utf8");

  expect.listEquals(testRussianUtf8, encodeUtf8(testRussianPhrase), "Russian to utf8");

  expect.listEquals(testGreekUtf8, encodeUtf8(testGreekPhrase), "Greek to utf8");

  expect.listEquals(testKatakanaUtf8, encodeUtf8(testKatakanaPhrase), "Katakana to utf8");
}

void testUtf8bytesToCodepoints() {
  expect.listEquals([954, 972, 963, 956, 949],
      utf8ToCodepoints([0xce, 0xba, 0xcf, 0x8c, 0xcf, 0x83, 0xce, 0xbc, 0xce, 0xb5]), "κόσμε");
  ...
  expect.listEquals([0x10000], utf8ToCodepoints([0xf0, 0x90, 0x80, 0x80]), "10000");
  expect.listEquals(
      [UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xf8, 0x88, 0x80, 0x80, 0x80]), "200000");
  expect.listEquals(
      [UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xfc, 0x84, 0x80, 0x80, 0x80, 0x80]), "4000000");
  ...
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xf7, 0xbf, 0xbf, 0xbf]), "1fffff");
  expect.listEquals(
      [UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xfb, 0xbf, 0xbf, 0xbf, 0xbf]), "3ffffff");
  expect.listEquals(
      [UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xfd, 0xbf, 0xbf, 0xbf, 0xbf, 0xbf]), "4000000");
  ...
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xef, 0xbf, 0xbd]), "fffd");
  expect.listEquals([0x10ffff], utf8ToCodepoints([0xf4, 0x8f, 0xbf, 0xbf]), "10ffff");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xf4, 0x90, 0x80, 0x80]), "110000");

  // unexpected continuation bytes
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0x80]), "80 => replacement character");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xbf]), "bf => replacement character");
  ...
  expect.listEquals(
      matchingReplacementChars, utf8ToCodepoints(allContinuationBytes), "80 - bf => replacement character x 64");
  ...
  expect.listEquals(matchingReplacementChars, utf8ToCodepoints(allFirstTwoByteSeq),
  ...
  expect.listEquals(matchingReplacementChars, utf8ToCodepoints(allFirstThreeByteSeq),
  ...
  expect.listEquals(
      matchingReplacementChars, utf8ToCodepoints(allFirstFourByteSeq), "f0 - f7 + space => replacement character x 8");
  ...
  expect.listEquals(
      matchingReplacementChars, utf8ToCodepoints(allFirstFiveByteSeq), "f8 - fb + space => replacement character x 4");
  ...
  expect.listEquals(
      matchingReplacementChars, utf8ToCodepoints(allFirstSixByteSeq), "fc - fd + space => replacement character x 2");

  // Sequences with last continuation byte missing
  expect.listEquals(
      [UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xc2]), "2-byte sequence with last byte missing");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xe0, 0x80]),
      "3-byte sequence with last byte missing");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xf0, 0x80, 0x80]),
      "4-byte sequence with last byte missing");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xf8, 0x88, 0x80, 0x80]),
      "5-byte sequence with last byte missing");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xfc, 0x80, 0x80, 0x80, 0x80]),
      "6-byte sequence with last byte missing");

  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xdf]),
      "2-byte sequence with last byte missing (hi)");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xef, 0xbf]),
      "3-byte sequence with last byte missing (hi)");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xf7, 0xbf, 0xbf]),
      "4-byte sequence with last byte missing (hi)");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xfb, 0xbf, 0xbf, 0xbf]),
      "5-byte sequence with last byte missing (hi)");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xfd, 0xbf, 0xbf, 0xbf, 0xbf]),
  ...
  // Impossible bytes
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xfe]), "fe");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xff]), "ff");
  expect.listEquals([
    UNICODE_REPLACEMENT_CHARACTER_CODEPOINT,
    UNICODE_REPLACEMENT_CHARACTER_CODEPOINT,
    UNICODE_REPLACEMENT_CHARACTER_CODEPOINT,
  ...
  // Overlong sequences
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xc0, 0xaf]), "c0 af");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xe0, 0x80, 0xaf]), "e0 80 af");
  expect
      .listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xf0, 0x80, 0x80, 0xaf]), "f0 80 80 af");
  expect.listEquals(
      [UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xf8, 0x80, 0x80, 0x80, 0xaf]), "f8 80 80 80 af");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xfc, 0x80, 0x80, 0x80, 0x80, 0xaf]),
      "fc 80 80 80 80 af");

  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xc1, 0xbf]), "c1 bf");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xe0, 0x9f, 0xbf]), "e0 9f bf");
  expect
      .listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xf0, 0x8f, 0xbf, 0xbf]), "f0 8f bf bf");
  expect.listEquals(
      [UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xf8, 0x87, 0xbf, 0xbf, 0xbf]), "f8 87 bf bf bf");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xfc, 0x83, 0xbf, 0xbf, 0xbf, 0xbf]),
      "fc 83 bf bf bf bf");

  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xc0, 0x80]), "c0 80");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xe0, 0x80, 0x80]), "e0 80 80");
  expect
      .listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xf0, 0x80, 0x80, 0x80]), "f0 80 80 80");
  expect.listEquals(
      [UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xf8, 0x80, 0x80, 0x80, 0x80]), "f8 80 80 80 80");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xfc, 0x80, 0x80, 0x80, 0x80, 0x80]),
      "fc 80 80 80 80 80");

  // Illegal code positions
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xed, 0xa0, 0x80]), "U+D800");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xed, 0xad, 0xbf]), "U+DB7F");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xed, 0xae, 0x80]), "U+DB80");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xed, 0xaf, 0xbf]), "U+DBFF");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xed, 0xb0, 0x80]), "U+DC00");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xed, 0xbe, 0x80]), "U+DF80");
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT], utf8ToCodepoints([0xed, 0xbf, 0xbf]), "U+DFFF");

  // Paired UTF-16 surrogates
  expect.listEquals([UNICODE_REPLACEMENT_CHARACTER_CODEPOINT, UNICODE_REPLACEMENT_CHARACTER_CODEPOINT],
...
void testUtf8BytesToString() {
  expect.stringEquals(testEnglishPhrase, decodeUtf8(testEnglishUtf8), "English");
  ...
  expect.stringEquals(testRussianPhrase, decodeUtf8(testRussianUtf8), "Russian");

  expect.stringEquals(testGreekPhrase, decodeUtf8(testGreekUtf8), "Greek");

  expect.stringEquals(testKatakanaPhrase, decodeUtf8(testKatakanaUtf8), "Katakana");
}
...
void testIterableMethods() {
  ...
  expect.stringEquals(testEnglishPhrase, new String.fromCharCodes(new List<int>.from(englishDecoder)));
  ...
  // get the whole translation using the Iterable interface
  expect.stringEquals(testKatakanaPhrase, new String.fromCharCodes(new List<int>.from(kataDecoder)));
}
