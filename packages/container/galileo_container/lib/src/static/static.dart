...
class StaticReflector extends Reflector {
  ...
  String? getName(Symbol symbol) {
    if (!names.containsKey(symbol)) {
      throw ArgumentError('The value of $symbol is unknown - it was not generated.');
    }
  ...
  @override
  ReflectedClass? reflectClass(Type clazz) => reflectType(clazz) as ReflectedClass?;
  ...
  ReflectedInstance? reflectInstance(Object object) {
    if (!instances.containsKey(object)) {
      throw ArgumentError('There is no reflection information available about $object.');
    }
  ...
  ReflectedType? reflectType(Type type) {
    if (!types.containsKey(type)) {
      throw ArgumentError('There is no reflection information available about $type.');
    }
