...
class GeneratedReflector extends Reflector {
  ...
  ReflectedType reflectType(Type type) {
    if (!reflectable.canReflectType(type)) {
      ...
      return mirror is ClassMirror ? new _GeneratedReflectedClass(mirror, this) : new _GeneratedReflectedType(mirror);
    }
  }
}
...
class _GeneratedReflectedInstance extends ReflectedInstance {
  ...
  _GeneratedReflectedInstance(this.mirror, this.reflector)
      : super(null, new _GeneratedReflectedClass(mirror.type, reflector), mirror.reflectee);
...
class _GeneratedReflectedClass extends ReflectedClass {
  ...
  @override
  List<ReflectedTypeParameter> get typeParameters => mirror.typeVariables.map(_convertTypeVariable).toList();

  @override
  List<ReflectedFunction> get constructors => _constructorsOf(mirror.declarations, reflector);

  @override
  List<ReflectedDeclaration> get declarations => _declarationsOf(mirror.declarations, reflector);

  @override
  List<ReflectedInstance> get annotations => mirror.metadata.map(reflector.reflectInstance).toList();
  ...
  @override
  ReflectedInstance newInstance(String constructorName, List positionalArguments,
      [Map<String, dynamic> namedArguments, List<Type> typeArguments]) {
    namedArguments ??= {};
    var result = mirror.newInstance(
        constructorName, positionalArguments, namedArguments.map((k, v) => new MapEntry(new Symbol(k), v)));
...
class _GeneratedReflectedType extends ReflectedType {
  ...
  _GeneratedReflectedType(this.mirror) : super(mirror.simpleName, null, mirror.reflectedType);
  ...
  ReflectedInstance newInstance(String constructorName, List positionalArguments,
      [Map<String, dynamic> namedArguments, List<Type> typeArguments]) {
    throw new UnsupportedError('Cannot create a new instance of $reflectedType.');
  }
}
...
class _GeneratedReflectedFunction extends ReflectedFunction {
  ...
  _GeneratedReflectedFunction(this.mirror, this.reflector, [this.closure])
      : super(
            ...
            [],
            null,
            !mirror.isRegularMethod ? null : new _GeneratedReflectedType(mirror.returnType),
...
List<ReflectedFunction> _constructorsOf(Map<String, DeclarationMirror> map, Reflector reflector) {
...
List<ReflectedDeclaration> _declarationsOf(Map<String, DeclarationMirror> map, Reflector reflector) {
  return map.entries.fold<List<ReflectedDeclaration>>([], (out, entry) {
    ...
    if (v is MethodMirror) {
      var decl = new ReflectedDeclaration(v.simpleName, v.isStatic, new _GeneratedReflectedFunction(v, reflector));
      return out..add(decl);
    } else {
      return out;
    }
...
ReflectedParameter _convertParameter(ParameterMirror mirror, Reflector reflector) {
  return new ReflectedParameter(mirror.simpleName, mirror.metadata.map(reflector.reflectInstance).toList(),
      reflector.reflectType(mirror.type.reflectedType), !mirror.isOptional, mirror.isNamed);
}
