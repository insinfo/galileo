# orm
[![Pub](https://img.shields.io/pub/v/galileo_orm.svg)](https://pub.dartlang.org/packages/galileo_orm)
...
Now you can combine the power and flexibility of galileo with a strongly-typed ORM.

Documentation for migrations can be found here:
https://galileo-dart.gitbook.io/galileo/v/2.x/orm/migrations
...
dependencies:
  galileo_orm: ^3.0.0
dev_dependencies:
  galileo_orm_generator: ^3.0.0
  build_runner: ^2.0.2 # Null safety
```

`package:galileo_orm_generator` exports a class that you can include
...
# Models
The ORM works best when used with `package:galileo_serialize`:

```dart
library galileo_orm.test.models.car;
...
Models can use the `@SerializableField()` annotation; `package:galileo_orm` obeys it.
...
abstract class _ThisIsNotAngalileoModel {
  @primaryKey
  String get username;
}
```
...
/// Returns an galileo plug-in that connects to a database, and sets up a controller connected to it...
galileoConfigurer connectToCarsTable(QueryExecutor executor) {
  return (galileo app) async {
    // Register the connection with galileo's dependency injection system.
    // 
...
class CarController extends Controller {
  ...
  createCar(QueryExecutor executor) async {
    // `package:galileo_orm` generates a strongly-typed `insert` function on the query class.
    ...
    // Auto-serialized using code generated by `package:galileo_serialize`
    return car;
  }
}
```
...
# Relations
`galileo_orm` supports the following relationships:
...
Test: https://raw.githubusercontent.com/galileo-dart/orm/master/galileo_orm_generator/test/many_to_many_test.dart
