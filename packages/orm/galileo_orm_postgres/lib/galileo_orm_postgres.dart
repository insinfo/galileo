...
class PostgreSqlExecutorPool extends QueryExecutor {
  ...
  PostgreSqlExecutorPool(this.size, this.connectionFactory, {this.logger}) : _pool = new Pool(size) {
  ...
  Future _open() async {
    if (_connections.isEmpty) {
      _connections.addAll(await Future.wait(new List.generate(size, (_) {
        ...
        return conn.open().then((_) => new PostgreSqlExecutor(conn, logger: logger));
      })));
    }
  }
