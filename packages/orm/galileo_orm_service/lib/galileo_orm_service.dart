import 'dart:async';
import 'package:galileo_framework/galileo_framework.dart' hide Query;
import 'package:galileo_orm/galileo_orm.dart';

/// A [Service] implementation that wraps over a [Query] class generated
/// via the galileo ORM.
class OrmService<Id, Data, TQuery extends Query<Data, QueryWhere>> extends Service<Id, Data> {
  ...
  SqlExpressionBuilder _findBuilder(TQuery query, String name) {
    return query.where.expressionBuilders.firstWhere((b) => b.columnName == name,
        orElse: () =>
            throw ArgumentError('${query.where.runtimeType} has no expression builder for a column named "$name".'));
  }
    ...
    if (allowQuery || !params.containsKey('provider')) {
      ...
      if (queryObj is Function(TQuery)) {
        ...
        queryObj.forEach((k, v) {
          ...
          } else if (k is String && v is! RequestContext && v is! ResponseContext) {
            _apply(query, k, v);
          }
        });
      }
  ...
  @override
  Future<List<Data>> readMany(List<Id> ids, [Map<String, dynamic> params]) async {
    if (ids.isEmpty) {
  ...
  @override
  Future<Data> findOne(
      [Map<String, dynamic> params, String errorMessage = 'No record was found matching the given query.']) async {
    var query = await queryCreator();
    await _applyQuery(query, params);
    var result = await query.getOne(executor);
    if (result != null) return result;
