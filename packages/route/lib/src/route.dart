...
class Route<T> {
  ...
  Route(this.path, {@required this.method, @required this.handlers})
      : _routeDefinition = RouteGrammar.routeDefinition.parse(SpanScanner(path.replaceAll(_straySlashes, ''))).value {
  ...
  factory Route.join(Route<T> a, Route<T> b) {
    ...
    return Route('$start/$end'.replaceAll(_straySlashes, ''), method: b.method, handlers: b.handlers);
  }
  ...
  Route<T> clone() {
    return Route<T>(path, method: method, handlers: handlers).._cache.addAll(_cache);
  }
