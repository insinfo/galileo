part of galileo_route.src.router;

class RouteGrammar {
  static const String notSlashRgx = r'([^/]+)';
  //static final RegExp rgx = RegExp(r'\((.+)\)');
  static final Parser<String> notSlash = match<String>(RegExp(notSlashRgx)).value((r) => r.span.text);

  static final Parser<Match> regExp = match<Match>(RegExp(r'\(([^\n)]+)\)([^/]+)?')).value((r) => r.scanner.lastMatch);
  ...
  static final Parser<ParsedParameterSegment> parsedParameterSegment = chain([
    match(RegExp(r'(int|num|double)'), errorMessage: 'Expected "int","double", or "num".').map((r) => r.span.text),
    parameterSegment,
  ]).map((r) {
    return ParsedParameterSegment(r.value[0] as String, r.value[1] as ParameterSegment);
  });

  static final Parser<WildcardSegment> wildcardSegment =
      match<WildcardSegment>(RegExp('$notSlashRgx?' r'\*' '$notSlashRgx?')).value((r) {
  ...
  static final Parser<ConstantSegment> constantSegment = notSlash.map<ConstantSegment>((r) => ConstantSegment(r.value));

  static final Parser<SlashSegment> slashSegment = match(SlashSegment.rgx).map((_) => SlashSegment());
  ...
  Parser<RouteResult> compile() {
    ...
    for (int i = 0; i < segments.length; i++) {
      ...
      if (out == null) {
        ...
        out = s.compileNext(out.then(match('/')).index(0).cast<RouteResult>(), isLast);
      }
    }
...
class WildcardSegment extends RouteSegment {
  ...
  @override
  Parser<RouteResult> compile(bool isLast) {
    return match(_compile(isLast)).map((r) => RouteResult({}, tail: r.scanner.lastMatch[1]));
  }
...
class OptionalSegment extends ParameterSegment {
  ...
  Parser<RouteResult> compileNext(Parser<RouteResult> p, bool isLast) {
    return p.then(_compile().opt()).map((r) {
      ...
      return (r.value[0] as RouteResult)..addAll({name: Uri.decodeComponent(r.value[1] as String)});
    });
  }
}
...
class ParameterSegment extends RouteSegment {
  ...
  Parser<String> _compile() {
    return regExp != null ? match<String>(regExp).value((r) => r.scanner.lastMatch[1]) : RouteGrammar.notSlash;
  }

  @override
  Parser<RouteResult> compile(bool isLast) {
    return _compile().map((r) => RouteResult({name: Uri.decodeComponent(r.value)}));
  }
...
class ParsedParameterSegment extends RouteSegment {
  ...
  @override
  Parser<RouteResult> compile(bool isLast) {
    return parameter._compile().map((r) => RouteResult({parameter.name: getValue(Uri.decodeComponent(r.span.text))}));
  }

  @override
  Parser<RouteResult> compileNext(Parser<RouteResult> p, bool isLast) {
    return p.then(parameter._compile()).map((r) {
      return (r.value[0] as RouteResult)..addAll({parameter.name: getValue(Uri.decodeComponent(r.value[1] as String))});
    });
  }
}
