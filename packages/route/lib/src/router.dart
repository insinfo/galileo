library galileo_route.src.router;
...
class Router<T> {
  ...
  Map<Pattern, Router<T>> get mounted => Map<Pattern, Router<T>>.unmodifiable(_mounted);

  List<Route<T>> get routes {
    return _routes.fold<List<Route<T>>>([], (out, route) {
      if (route is SymlinkRoute<T>) {
        var childRoutes = route.router.routes.fold<List<Route<T>>>([], (out, r) {
          return out
            ..add(
  ...
  Route<T> addRoute(String method, String path, T handler, {Iterable<T> middleware}) {
    middleware ??= <T>[];
    if (_useCache == true) {
  ...
  void dumpTree({callback(String tree), String header = 'Dumping route tree:', String tab = '  '}) {
  ...
  Future<SymlinkRoute<T>> groupAsync(String path, FutureOr<void> callback(Router<T> router),
  ...
  String navigate(Iterable linkParams, {bool absolute = true}) {
    ...
    for (final param in linkParams) {
      ...
      if (param is String) {
        ...
        if (!resolved) {
          throw RoutingException('Cannot resolve route for link param "$param".');
        }
      } else if (param is Route) {
        segments.add(param.path.replaceAll(_straySlashes, ''));
      } else if (param is Map<String, dynamic>) {
        if (lastRoute == null) {
          throw RoutingException('Maps in link params must be preceded by a Route or String.');
        } else {
          segments.removeLast();
          segments.add(lastRoute.makeUri(param).replaceAll(_straySlashes, ''));
        }
      } else {
        throw RoutingException('Link param $param is not Route, String, or Map<String, dynamic>.');
      }
    }

    return absolute ? '/${segments.join('/').replaceAll(_straySlashes, '')}' : segments.join('/');
  }
  ...
  bool resolve(String absolute, String relative, List<RoutingResult<T>> out,
    ...
    final cleanRelative = strip == false ? relative : stripStraySlashes(relative);
  ...
  Iterable<RoutingResult<T>> resolveAbsolute(String path, {String method = 'GET', bool strip = true}) =>
      resolveAll(path, path, method: method, strip: strip);

  /// Finds every possible [Route] that matches the given path,
  /// with the given method.
  Iterable<RoutingResult<T>> resolveAll(String absolute, String relative, {String method = 'GET', bool strip = true}) {
    if (_useCache == true) {
      return _cache.putIfAbsent(
          '$method$absolute', () => _resolveAll(absolute, relative, method: method, strip: strip));
    }

    return _resolveAll(absolute, relative, method: method, strip: strip);
  }

  Iterable<RoutingResult<T>> _resolveAll(String absolute, String relative, {String method = 'GET', bool strip = true}) {
...
class _ChainedRouter<T> extends Router<T> {
  ...
  @override
  Route<T> addRoute(String method, String path, handler, {Iterable<T> middleware}) {
    var route = super.addRoute(method, path, handler, middleware: []..addAll(_handlers)..addAll(middleware ?? []));
  ...
  @override
  SymlinkRoute<T> group(String path, void callback(Router<T> router), {Iterable<T> middleware, String name}) {
    final router = _ChainedRouter<T>(_root, []..addAll(_handlers)..addAll(middleware ?? []));
    callback(router);
    return mount(path, router)..name = name;
  }
  ...
  for (var route in router.routes) {
    if (route is SymlinkRoute<T>) {
      ...
      for (var route in child.routes) {
        ...
        flattened.addRoute(route.method, joined.replaceAll(_straySlashes, ''), route.handlers.last,
