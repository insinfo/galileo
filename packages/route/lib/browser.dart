...
abstract class _BrowserRouterImpl<T> extends Router<T> implements BrowserRouter<T> {
  bool _listening = false;
  Route _current;
  StreamController<RoutingResult<T>> _onResolve = StreamController<RoutingResult<T>>();
  ...
  Route on(String path, T handler, {Iterable<T> middleware}) => all(path, handler, middleware: middleware);

  void prepareAnchors() {
    final anchors = window.document.querySelectorAll('a').cast<AnchorElement>(); //:not([dynamic])');
...
class _PushStateRouter<T> extends _BrowserRouterImpl<T> {
  ...
  void _goTo(String uri) {
    ...
    if (resolved == null) {
      ...
      window.history.pushState({'path': route.path, 'params': {}}, route.name ?? route.path, relativeUri);
      _onResolve.add(resolved);
