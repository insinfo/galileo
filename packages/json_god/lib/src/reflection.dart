library galileo_json_god.reflection;

import 'dart:mirrors';
import 'package:galileo_json_god/galileo_json_god.dart';
...
List<Symbol> _findGetters(ClassMirror classMirror) {
  ...
  classMirror.instanceMembers.forEach((Symbol symbol, MethodMirror methodMirror) {
    if (methodMirror.isGetter && symbol != hashCodeSymbol && symbol != runtimeTypeSymbol) {
...
deserialize(value, Type outputType, Deserializer deserializer, [@deprecated bool debug = false]) {
  ...
  try {
    if (value is List) {
      ...
      if (typeArguments.isEmpty) {
        ...
        it = value.map((item) => deserializer(item, outputType: typeArguments[0].reflectedType));
      }

      if (typeArguments.isEmpty) return it.toList();
      logger.info('Casting list elements to ${typeArguments[0].reflectedType} via List.from');
      ...
      if (mirror is ClassMirror) {
        ...
        throw new ArgumentError('${typeArguments[0].reflectedType} is not a class.');
      }
...
_deserializeFromJsonByReflection(data, Deserializer deserializer, Type outputType, [@deprecated bool debug = false]) {
  // Check for fromJson
  ...
  var fromJson = new Symbol('${MirrorSystem.getName(type.simpleName)}.fromJson');
  ...
  if (classMirror.isSubclassOf(reflectClass(Map))) {
    ...
    if (typeArguments.isEmpty || classMirror.typeArguments.every((t) => t == currentMirrorSystem().dynamicType)) {
      return data;
    } else {
      var mapType = reflectType(Map, typeArguments.map((t) => t.reflectedType).toList()) as ClassMirror;
    ...
    data.keys.forEach((key) {
      try {
        ...
        logger.info("I want to set $key to the following ${deserializedValue.runtimeType}: $deserializedValue");
        // Get target type of getter
        Symbol searchSymbol = new Symbol(key.toString());
        Symbol symbolForGetter = classMirror.instanceMembers.keys.firstWhere((x) => x == searchSymbol);
        Type requiredType = classMirror.instanceMembers[symbolForGetter].returnType.reflectedType;
        if (data[key].runtimeType != requiredType) {
          ...
          deserializedValue = deserializer(deserializedValue, outputType: requiredType);
        }

        logger.info("Final deserialized value for $key: $deserializedValue <${deserializedValue.runtimeType}>");
