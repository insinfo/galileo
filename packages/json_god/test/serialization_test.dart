...
main() {
  ...
  group('serialization', () {
    ...
    test('serialize with schema validation', testSerializationWithSchemaValidation);
  });
}
...
testSerializationOfDates() {
  ...
  var deserialized = jsonDecode(s);
  expect(deserialized['date'], equals(date.toIso8601String()));
}

testSerializationOfMaps() {
  var simple = json.decode(god.serialize({'hello': 'world', 'one': 1, 'class': new SampleClass('world')}));
...
testSerializationWithSchemaValidation() async {
  BabelRc babelRc = new BabelRc(presets: ['es2015', 'stage-0'], plugins: ['add-module-exports']);
