...
/// Handles authentication within an Galileo application.
class GalileoAuth<User> {
  Hmac _hs256;
  int _jwtLifeSpan;
  final StreamController<User> _onLogin = StreamController<User>(), _onLogout = StreamController<User>();
  ...
  String _randomString(
      {int length = 32, String validChars = "ABCDEFHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_"}) {
    var chars = <int>[];
  ...
  /// Configures an Galileo server to decode and validate JSON Web tokens on demand,
  /// whenever an instance of [User] is injected.
  Future<void> configureServer(Galileo app) async {
    if (serializer == null)
      throw StateError(
          'An `GalileoAuth` plug-in was called without its `serializer` being set. All authentication will fail.');
    if (deserializer == null)
      throw StateError(
          'An `GalileoAuth` plug-in was called without its `deserializer` being set. All authentication will fail.');

    app.container.registerSingleton(this);
    if (runtimeType != GalileoAuth) app.container.registerSingleton(this, as: GalileoAuth);

    if (!app.container.has<_AuthResult<User>>()) {
      app.container.registerLazySingleton<Future<_AuthResult<User>>>((container) async {
  ...
  void _apply(RequestContext req, ResponseContext res, AuthToken token, User user) {
  ...
  ///
  /// Now that `package:galileo_framework` supports asynchronous injections, this middleware
  /// is no longer directly necessary. Instead, call [configureServer]. You can then use
  /// `makeAsync<User>`, or Galileo's injections directly:
  ///
  /// ```dart
  ...
  ///   // `package:Galileo_auth` decodes the JWT on demand.
  ///   print(user.name);
  /// });
  ///
  /// @Expose('/my')
  ...
  Future<_AuthResult<User>> _decodeJwt(RequestContext req, ResponseContext res) async {
    ...
    if (jwt != null) {
      ...
      if (enforceIp) {
        if (req.ip != null && req.ip != token.ipAddress)
          throw GalileoHttpException.forbidden(message: "JWT cannot be accessed from this IP address.");
      }

      if (token.lifeSpan > -1) {
        var expiry = token.issuedAt.add(Duration(milliseconds: token.lifeSpan.toInt()));

        if (!expiry.isAfter(DateTime.now())) throw GalileoHttpException.forbidden(message: "Expired JWT.");
      }
  ...
  String getJwt(RequestContext req) {
    if (req.headers.value("Authorization") != null) {
      ...
      if (_rgxBearer.hasMatch(authHeader)) return authHeader.replaceAll(_rgxBearer, "").trim();
    } else if (allowCookie && req.cookies.any((cookie) => cookie.name == "token")) {
      return req.cookies.firstWhere((cookie) => cookie.name == "token").value;
    } else if (allowTokenInQuery && req.uri.queryParameters['token'] is String) {
  ...
  Cookie protectCookie(Cookie cookie) {
    ...
    if (_jwtLifeSpan > 0) {
      ...
      cookie.expires ??= DateTime.now().add(Duration(milliseconds: _jwtLifeSpan));
    }
  ...
  Future<Map<String, dynamic>> reviveJwt(RequestContext req, ResponseContext res) async {
    try {
      ...
      if (jwt == null) {
        throw GalileoHttpException.forbidden(message: "No JWT provided");
      } else {
        var token = AuthToken.validate(jwt, _hs256);
        if (enforceIp) {
      ...
      if (e is GalileoHttpException) rethrow;
      throw GalileoHttpException.badRequest(message: "Malformed JWT");
    }
  }
  ...
  RequestHandler authenticate(type, [GalileoAuthOptions<User> options]) {
    return (RequestContext req, ResponseContext res) async {
      List<String> names = [];
      var arr = type is Iterable ? type.map((x) => x.toString()).toList() : [type.toString()];

      for (String t in arr) {
        var n = t.split(',').map((s) => s.trim()).where((String s) => s.isNotEmpty).toList();
        names.addAll(n);
      }
      ...
      for (int i = 0; i < names.length; i++) {
        ...
        var strategy = strategies[name] ??= throw ArgumentError('No strategy "$name" found.');

        var hasExisting = req.container.has<User>();
        var result = hasExisting ? req.container.make<User>() : await strategy.authenticate(req, res, options);
        if (result == true)
          return result;
          ...
          // Create JWT
          var token = AuthToken(userId: userId, lifeSpan: _jwtLifeSpan, ipAddress: req.ip);
          ...
          if (options?.successRedirect?.isNotEmpty == true) {
            ...
            return false;
          } else if (options?.canRespondWithJson != false && req.accepts('application/json')) {
            var user = hasExisting ? result : await deserializer(await serializer(result));
            _onLogin.add(user);
          ...
          if (res.statusCode == 301 || res.statusCode == 302 || res.headers.containsKey('location'))
            return false;
  ...
  RequestHandler logout([GalileoAuthOptions<User> options]) {
    return (RequestContext req, ResponseContext res) async {
      if (req.container.has<User>()) {
      ...
      if (options != null && options.successRedirect != null && options.successRedirect.isNotEmpty) {
        await res.redirect(options.successRedirect);
      }
