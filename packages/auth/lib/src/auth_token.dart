...
class AuthToken {
  final SplayTreeMap<String, String> _header = SplayTreeMap.from({"alg": "HS256", "typ": "JWT"});
  ...
  AuthToken({this.ipAddress, this.lifeSpan = -1, this.userId, DateTime issuedAt, Map payload = const {}}) {
    this.issuedAt = issuedAt ?? DateTime.now();
    this.payload.addAll(payload?.keys?.fold({}, (out, k) => out..[k.toString()] = payload[k]) ?? {});
  }

  factory AuthToken.fromJson(String jsons) => AuthToken.fromMap(json.decode(jsons) as Map);
  ...
  factory AuthToken.parse(String jwt) {
    ...
    if (split.length != 3) throw GalileoHttpException.notAuthenticated(message: "Invalid JWT.");
  ...
  factory AuthToken.validate(String jwt, Hmac hmac) {
    ...
    if (signature != split[2])
      throw GalileoHttpException.notAuthenticated(message: "JWT payload does not match hashed version.");
