...
class LocalAuthStrategy<User> extends AuthStrategy<User> {
  ...
  LocalAuthStrategy(this.verifier,
      ...
      String this.invalidMessage = 'Please provide a valid username and password.',
      bool this.allowBasic = true,
      bool this.forceBasic = false,
      String this.realm = 'Authentication is required.'});

  @override
  Future<User> authenticate(RequestContext req, ResponseContext res, [GalileoAuthOptions options_]) async {
    GalileoAuthOptions options = options_ ?? GalileoAuthOptions();
    ...
    if (allowBasic) {
      ...
      if (_rgxBasic.hasMatch(authHeader)) {
        ...
        String authString = String.fromCharCodes(base64.decode(base64AuthString));
        if (_rgxUsrPass.hasMatch(authString)) {
          Match usrPassMatch = _rgxUsrPass.firstMatch(authString);
          verificationResult = await verifier(usrPassMatch.group(1), usrPassMatch.group(2));
        } else
          throw GalileoHttpException.badRequest(errors: [invalidMessage]);
    ...
    if (verificationResult == null) {
      var body = await req.parseBody().then((_) => req.bodyAsMap).catchError((_) => <String, dynamic>{});
      if (_validateString(body[usernameField]?.toString()) && _validateString(body[passwordField]?.toString())) {
        verificationResult = await verifier(body[usernameField]?.toString(), body[passwordField]?.toString());
      }
    }

    if (verificationResult == false || verificationResult == null) {
      if (options.failureRedirect != null && options.failureRedirect.isNotEmpty) {
