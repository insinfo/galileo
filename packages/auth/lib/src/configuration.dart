...
class ExternalAuthOptions {
  ...
  ExternalAuthOptions._(this.clientId, this.clientSecret, this.redirectUri, this.scopes) {
    if (clientId == null) {
  ...
  factory ExternalAuthOptions(
    ...
    if (redirectUri is String) {
      ...
      throw ArgumentError.value(redirectUri, 'redirectUri', 'must be a String or Uri');
    }
  }
  ...
  factory ExternalAuthOptions.fromMap(Map map) {
    return ExternalAuthOptions(
      ...
      scopes: map['scopes'] is Iterable ? ((map['scopes'] as Iterable).map((x) => x.toString())) : <String>[],
    );
  }
  ...
  ExternalAuthOptions copyWith({String clientId, String clientSecret, redirectUri, Iterable<String> scopes}) {
    return ExternalAuthOptions(
  ...
  String toString({bool obscureSecret = true, int asteriskCount}) {
    ...
    if (!obscureSecret) {
      ...
      var codeUnits = List<int>.filled(asteriskCount ?? clientSecret.length, $asterisk);
