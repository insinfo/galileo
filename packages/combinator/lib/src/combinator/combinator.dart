...
abstract class Parser<T> {
  ...
  ParseResult<T> _parse(ParseArgs args) {
    ...
    if (args.trampoline.hasMemoized(this, pos)) return args.trampoline.getMemoized<T>(this, pos);

    if (args.trampoline.isActive(this, pos)) return new ParseResult(args.trampoline, args.scanner, this, false, []);
  ...
  Parser<T> check(Matcher matcher, {String errorMessage, SyntaxErrorSeverity severity}) =>
      new _Check<T>(this, matcher, errorMessage, severity ?? SyntaxErrorSeverity.error);
  ...
  Parser<T> safe({bool backtrack: true, String errorMessage, SyntaxErrorSeverity severity}) =>
      new _Safe<T>(this, backtrack, errorMessage, severity ?? SyntaxErrorSeverity.error);
  ...
  Parser<List<T>> separatedBy(Parser other) {
    ...
    return this.then(suffix.star()).map((r) {
      var preceding = r.value.isEmpty ? [] : (r.value[0] == null ? [] : [r.value[0]]);
  ...
  Parser<T> surroundedByCurlyBraces({T defaultValue}) =>
      opt().surroundedBy(match('{').space(), match('}').space()).map((r) => r.value ?? defaultValue);

  Parser<T> surroundedBySquareBrackets({T defaultValue}) =>
      opt().surroundedBy(match('[').space(), match(']').space()).map((r) => r.value ?? defaultValue);
  ...
  Parser<T> parenthesized() => surroundedBy(match('(').space(), match(')').space());

  /// Consumes any trailing whitespace.
  Parser<T> space() => trail(new RegExp(r'[ \n\r\t]+'));

  /// Consumes 0 or more instance(s) of this parser.
  ListParser<T> star({bool backtrack: true}) => times(1, exact: false, backtrack: backtrack).opt();
  ...
  ListParser<T> times(int count,
      {bool exact: true, String tooFew, String tooMany, bool backtrack: true, SyntaxErrorSeverity severity}) {
    return new _Repeat<T>(this, count, exact, tooFew, tooMany, backtrack, severity ?? SyntaxErrorSeverity.error);
  }
...
abstract class ListParser<T> extends Parser<List<T>> {
  ...
  Parser<List<T>> where(bool Function(T) f) => map<List<T>>((r) => r.value.where(f).toList());
...
class ParseResult<T> {
  ...
  ParseResult(this.trampoline, this.scanner, this.parser, this.successful, this.errors, {this.span, this.value});

  ParseResult<T> change({Parser<T> parser, bool successful, Iterable<SyntaxError> errors, FileSpan span, T value}) {
