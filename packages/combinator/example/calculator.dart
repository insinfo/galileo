...
Parser<num> calculatorGrammar() {
  ...
  var number = match<num>(new RegExp(r'-?[0-9]+(\.[0-9]+)?')).value((r) => num.parse(r.span.text));

  var hex = match<int>(new RegExp(r'0x([A-Fa-f0-9]+)')).map((r) => int.parse(r.scanner.lastMatch[1], radix: 16));

  var binary = match<int>(new RegExp(r'([0-1]+)b')).map((r) => int.parse(r.scanner.lastMatch[1], radix: 2));
