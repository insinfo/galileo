# redis
[![Pub](https://img.shields.io/pub/v/galileo_redis.svg)](https://pub.dartlang.org/packages/galileo_redis)
[![build status](https://travis-ci.org/galileo-dart/redis.svg)](https://travis-ci.org/galileo-dart/redis)

Redis-enabled services for the Galileo framework.
`RedisService` can be used alone, *or* as the backend of a
[`CacheService`](https://github.com/galileo-dart/cache),
...
## Notes
* Neither `index`, nor `modify` is atomic; each performs two separate queries.`galileo_redis` stores data as JSON strings, rather than as Redis hashes, so an update-in-place is impossible.
...
when caching another database, you'll preserve the ID or primary key of an item. `galileo_redis` heeds this. If no
