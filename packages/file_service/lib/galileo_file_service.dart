...
class JsonFileService extends Service<String, Map<String, dynamic>> {
  ...
  JsonFileService(this.file, {bool allowRemoveAll: false, bool allowQuery: true, MapService store}) {
    _store = store ?? new MapService(allowRemoveAll: allowRemoveAll == true, allowQuery: allowQuery != false);
  }

  Map<String, dynamic> _coerceStringDynamic(Map m) {
    return m.keys.fold<Map<String, dynamic>>(<String, dynamic>{}, (out, k) => out..[k.toString()] = m[k]);
  }
  ...
  Future _load() {
    return _mutex.withResource(() async {
      ...
      if (_lastStat == null || stat.modified.millisecondsSinceEpoch > _lastStat.modified.millisecondsSinceEpoch) {
        _lastStat = stat;
        ...
        _store.items.addAll(list.map((x) => _coerceStringDynamic(_revive(x) as Map))); // Insert all new entries
      }
    });
  }
  ...
  _save() {
    return _mutex.withResource(() {
      return file.writeAsString(json.encode(_store.items.map(_jsonify).toList()));
    });
  }
  ...
  Future<List<Map<String, dynamic>>> index([Map<String, dynamic> params]) async =>
      _load().then((_) => _store.index(params)).then((it) => it.map(_jsonifyToSD).toList());
  ...
  @override
  Future<Map<String, dynamic>> create(data, [Map<String, dynamic> params]) async {
    await _load();
  ...
  @override
  Future<Map<String, dynamic>> update(id, data, [Map<String, dynamic> params]) async {
    await _load();
  ...
  @override
  Future<Map<String, dynamic>> modify(id, data, [Map<String, dynamic> params]) async {
    await _load();
    var r = await _store.update(id, data, params).then(_jsonifyToSD);
    await _save();
    return r;
...
Map<String, dynamic> _jsonifyToSD(Map<String, dynamic> map) => _jsonify(map).cast<String, dynamic>();

dynamic _revive(x) {
  if (x is Map) {
    return x.keys.fold<Map<String, dynamic>>({}, (out, k) => out..[k.toString()] = _revive(x[k]));
  } else if (x is Iterable)
