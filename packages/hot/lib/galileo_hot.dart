...
class HotReloader {
  ...
  /// Invoked to load a new instance of [Galileo] on file changes.
  final FutureOr<Galileo> Function() generator;
  ...
  HotReloader(this.generator, Iterable paths,
      {Duration timeout, this.vmServiceHost = 'localhost', this.vmServicePort = 8181, this.enableHotkeys = true}) {
  ...
  void sendError(HttpRequest request, int status, String title_, e) {
    ...
    response.headers
      ...
      ..set(HttpHeaders.serverHeader, 'galileo_hot');

    if (request.headers.value(HttpHeaders.acceptEncodingHeader)?.toLowerCase()?.contains('gzip') == true) {
      response
  ...
  Future<HttpServer> startServer([address, int port]) async {
    ...
    if (_paths?.isNotEmpty != true) {
      _logWarning('You have instantiated a HotReloader without providing any filesystem paths to watch.');
    }
    ...
    if (enableHotkeys) {
      var serverUri = Uri(scheme: 'http', host: server.address.address, port: server.port);
      var observatoryUri = await dev.Service.getInfo().then((i) => i.serverUri);

      print(styleBold.wrap(
          '\n🔥  To hot reload changes while running, press "r". To hot restart (and rebuild state), press "R".'));
      stdout.write('Your Galileo server is listening at: ');
      print(wrapWith('$serverUri', [cyan, styleUnderlined]));
      stdout.write('An Observatory debugger and profiler on ${Platform.operatingSystem} is available at: ');
      print(wrapWith('$observatoryUri', [cyan, styleUnderlined]));
      print('For a more detailed help message, press "h". To quit, press "q".\n');

      if (_paths.isNotEmpty) {
        print(darkGray.wrap('Changes to the following path(s) will also trigger a hot reload:'));
      ...
      try {
        sub = stdin.expand((l) => l).listen((ch) async {
          if (ch == $r) {
            _handleWatchEvent(WatchEvent(ChangeType.MODIFY, '[manual-reload]'), isHot);
          }
          if (ch == $R) {
            ...
            print('Press "r" to hot reload the Dart VM, and restart the active server.');
            print('Press "R" to restart the server, WITHOUT a hot reload of the VM.');
  ...
  Future _killServer() async {
    if (_server != null) {
      ...
      scheduleMicrotask(() async {
        ...
        try {
          var ws = _server.app.container.make<GalileoWebSocket>();

          for (var client in ws.clients) {
            try {
              await client.close();
            } catch (e) {
              _logWarning('Couldn\'t close WebSocket from session #${client.request.session.id}: $e');
            }
          }
  ...
  void _handleWatchEvent(WatchEvent e, [bool hot = true]) async {
    ...
    if (hot) {
      ...
      if (!report.success) {
        _logWarning('Hot reload failed - perhaps some sources have not been generated yet.');
      }
    }
