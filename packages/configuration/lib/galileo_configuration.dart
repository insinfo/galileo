...
Future<void> _loadYamlFile(Map map, File yamlFile, Map<String, String> env, void Function(String msg) warn) async {
  if (await yamlFile.exists()) {
    var config = loadYaml(await yamlFile.readAsString());

    if (config is! Map) {
      warn('The configuration at "${yamlFile.absolute.path}" is not a Map. Refusing to load it.');
      return;
    }
      ...
      for (var inc in includeList) {
        if (inc is! String) {
          ...
          if (!await includeFile.exists()) {
            warn('Included configuration file "$includeFilePath" does not exist.');
          } else {
...
Object _applyEnv(var v, Map<String, String> env, void Function(String msg) warn) {
  if (v is String) {
...
Future<Map> loadStandaloneConfiguration(FileSystem fileSystem,
  ...
  var environmentName = env['GALILEO_ENV'] ?? 'development';
...
GalileoConfigurer configuration(FileSystem fileSystem,
  ...
  return (Galileo app) async {
    var config = await loadStandaloneConfiguration(
      ...
      envPath: envPath,
      onWarning: app.logger == null ? null : (msg) => app.logger?.warning('WARNING: $msg'),
    );
