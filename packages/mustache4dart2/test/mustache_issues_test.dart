...
import 'package:galileo_mustache4dart/galileo_mustache4dart.dart';
...
void main() {
  group('mustache4dart issues', () {
    test('#9: use empty strings for non existing variable',
        () => expect(render("{{#sec}}[{{variable}}]{{/sec}}", {'sec': 42}), '[]'));
    ...
    group('#16', () {
      test('side effect', () => expect(render('{{^x}}x{{/x}}!!!', null), 'x!!!'));

      test('root cause: For null objects the value of any property should be null', () {
    ...
    group('#17', () {
      test('side effect', () => expect(render('{{#a}}[{{{a}}}|{{b}}]{{/a}}', {'a': 'aa', 'b': 'bb'}), '[aa|bb]'));
    ...
    test('#20', () {
      ...
      final template = new File("$currentPath/lorem-ipsum.txt").readAsStringSync();
    ...
    test('#28', () {
      ...
      expect(render('{{#children}}{{name}}{{#hasChildren}} has children{{/hasChildren}},{{/children}}', model),
...
{{#children}}{{foo}}!{{/children}}''';
    ...
    test('#44 should provide a way to check for non empty lists', () {
      ...
      expect(render('{{^list.empty}}<ul>{{#list}}<li>{{.}}</li>{{/list}}</ul>{{/list.empty}}', map),
