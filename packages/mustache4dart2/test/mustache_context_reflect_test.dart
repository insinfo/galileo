...
void main() {
  group('reflect', () {
    ...
    group('field([name])', () {
      ...
      group(".exists", () {
        ...
        test('returns false if [] operator returns a null value', () {
          expect(reflect(new ClassWithBrackets()).field('nullval').exists, isFalse);
        });
      ...
      group(".val()", () {
        ...
        test('does not returns the value of a get methods', () {
          final george = new Person("George", lastname: "Valotasios", parent: new Person("Thomas"));
        ...
        test('returns the value from a [] operator', () {
          ...
          expect(actual.val(), isA<Person>());
          expect(actual.val().name, 'xyz');
        }, onPlatform: {"js": new Skip("[] operator can not be reflected in javascript")});

        test('returns always a reference to the value', () {
          final thomas = new Person("Thomas");
          final george = new Person("George", lastname: "Valotasios", parent: thomas);
        ...
        test('returns a ref to the function if it has an arity of 1', () {
          ...
          expect(actual.val(), isA<Function>());
          expect(actual.val()("-"), "[[- 1]]");
        });
      });
    });
    ...
    test('does not use reflection with Maps', () {
      ...
      expect(reflection, isNot(isA<Mirror>()));
    });
