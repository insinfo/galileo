...
void main() {
  group('mustache4dart tests', () {
    ...
    test('Compiled function with existing context same with render',
        () => expect(salut({'name': 'George'}), render(salutTemplate, {'name': 'George'})));
    ...
    test(
        'Template with string context after closing one', () => expect(render('{{^x}}No x{{/x}}!!!', null), 'No x!!!'));
    ...
    test(
        'Deeper context test',
        () => expect(render('{{#a}}{{one}}{{#b}}-{{one}}{{two}}{{#c}}-{{one}}{{two}}{{three}}{{/c}}{{/b}}{{/a}}', map),
            '1-12-123'));
...
''';
      ...
      test('Provide lambdas as a method() within a class', () {
        ...
        final template = '''{{#map.things}}{{#lambda3}}{{name}}{{/lambda3}}|{{/map.things}}''';
