...
import 'package:galileo_mustache4dart/mustache_context.dart';
...
void main() {
  ...
  test('Object with iterables', () {
    ...
    p.contactInfos
        .add(new _ContactInfo('Address', {'Street': 'Κολοκωτρόνη', 'Num': '31', 'Zip': '42100', 'Country': 'GR'}));
  ...
  test('Deep subcontext test', () {
    ...
    expect(ctx.field('a').field('b'), isNotNull, reason: "a.b should exists when using $map");
    expect(ctx.field('a').field('b').field('one').value(), '1', reason: "a.b.one == a.own when using $map");
    expect(ctx.field('a').field('b').field('two').value(), '2', reason: "a.b.two == b.two when using $map");
    expect(ctx.field('a').field('b').field('three'), isNull);
    expect(ctx.field('a').field('b').field('c'), isNotNull, reason: "a.b.c should not be null when using $map");

    var abc = ctx.field('a').field('b').field('c');
    expect(abc.field('one').value(), '1', reason: "a.b.c.one == a.one when using $map");
    expect(abc.field('two').value(), '2', reason: "a.b.c.two == b.two when using $map");
