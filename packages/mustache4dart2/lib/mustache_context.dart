...
import 'package:galileo_mustache4dart/src/mirrors.dart';
...
_createMustacheContext(obj, {MustacheContext parent, bool assumeNullNonExistingProperty}) {
  ...
  return new _MustacheContext(obj, parent: parent, assumeNullNonExistingProperty: assumeNullNonExistingProperty);
}
...
class _MustacheContext implements MustacheContext {
  ...
  _MustacheContext(this.ctx, {_MustacheContext this.parent, this.assumeNullNonExistingProperty});
...
class _IterableMustacheContextDecorator extends IterableBase<_MustacheContext> implements MustacheContext {
  ...
  _IterableMustacheContextDecorator(this.ctx, {this.parent, this.assumeNullNonExistingProperty});

  value([arg]) => throw new Exception('Iterable can not be called as a function');

  Iterator<_MustacheContext> get iterator => new _MustacheContextIteratorDecorator(ctx.iterator,
      parent: parent, assumeNullNonExistingProperty: assumeNullNonExistingProperty);
  ...
  field(String key) {
    assert(key == DOT); // 'Iterable can only be iterated. No [] implementation is available'
    return this;
  }
...
class _MustacheContextIteratorDecorator extends Iterator<_MustacheContext> {
  ...
  _MustacheContextIteratorDecorator(this.delegate, {this.parent, this.assumeNullNonExistingProperty});
