...
void main() {
  ...
  test('timeout connect test', () async {
    ...
    try {
      ...
      await MySqlConnection.connect(ConnectionSettings(port: 12346, timeout: Duration(microseconds: 5)));
    } on TimeoutException {
      thrown = true;
    } on SocketException {
      thrown = true;
  ...
  test('socket too many connections on connect', () async {
    ...
    try {
      ...
      sock.listen((socket) async {
        socket.add([23, 0, 0, 0]);
        socket.add(
            [255, 16, 4, 84, 111, 111, 32, 109, 97, 110, 121, 32, 99, 111, 110, 110, 101, 99, 116, 105, 111, 110, 115]);
        socket.close();
      });
