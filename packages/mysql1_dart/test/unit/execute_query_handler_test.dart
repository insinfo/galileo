...
library galileo_mysql.test.unit.execute_query_handler_test;

import 'dart:convert';

import 'package:mocktail/mocktail.dart';
import 'package:galileo_mysql/src/blob.dart';
import 'package:galileo_mysql/src/prepared_statements/execute_query_handler.dart';

import 'package:galileo_mysql/src/prepared_statements/prepared_query.dart';
...
void main() {
  group('ExecuteQueryHandler.createNullMap', () {
    ...
    test('can build map with eight nulls', () {
      var handler = ExecuteQueryHandler(null, false, [null, null, null, null, null, null, null, null]);
      var nullmap = handler.createNullMap();
    ...
    test('can build map with some nulls and some not', () {
      var handler = ExecuteQueryHandler(null, false, [null, 0, 0, 0, 0, 0, 0, null]);
      var nullmap = handler.createNullMap();
    ...
    test('can build map which is more than one byte', () {
      var handler = ExecuteQueryHandler(null, false, [null, 0, 0, 0, 0, 0, 0, null, 0, 0, 0, 0, 0, 0, 0, 0]);
      var nullmap = handler.createNullMap();
      expect(nullmap, equals([129, 0]));
    });

    test('can build map which just is more than one byte', () {
      var handler = ExecuteQueryHandler(null, false, [null, 0, 0, 0, 0, 0, 0, null, 0]);
      var nullmap = handler.createNullMap();
      expect(nullmap, equals([129, 0]));
    });

    test('can build map which just is more than one byte with a null', () {
      var handler = ExecuteQueryHandler(null, false, [null, 0, 0, 0, 0, 0, 0, null, null]);
      var nullmap = handler.createNullMap();
      expect(nullmap, equals([129, 1]));
    });

    test('can build map which just is more than one byte with a null, another pattern', () {
      var handler = ExecuteQueryHandler(null, false, [null, 0, null, 0, 0, 0, 0, null, null]);
      var nullmap = handler.createNullMap();
  ...
  group('ExecuteQueryHandler.prepareValue', () {
    ...
    test('can prepare blob values correctly', () {
      expect(handler.prepareValue(Blob.fromString('hello')), equals(utf8.encode('hello')));
    });
  });
