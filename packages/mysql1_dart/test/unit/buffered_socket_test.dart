...
void main() {
  group('buffered socket', () {
    ...
    test('can read data which is already available', () async {
      ...
      var socket;
      var thesocket = await BufferedSocket.connect('localhost', 100, const Duration(seconds: 5), onDataReady: () async {
        var buffer = Buffer(4);
    ...
    test('can read data which is not yet available', () async {
      var c = Completer();
      var socket = await BufferedSocket.connect('localhost', 100, const Duration(seconds: 5),
    ...
    test('can read data which is not yet available, arriving in two chunks', () async {
      var c = Completer();
      var socket = await BufferedSocket.connect('localhost', 100, const Duration(seconds: 30),
          onDataReady: () {}, onDone: () {}, onError: (e) {}, socketFactory: factory);
      var buffer = Buffer(4);
      unawaited(socket.readBuffer(buffer).then((_) {
        expect(buffer.list, equals([1, 2, 3, 4]));
