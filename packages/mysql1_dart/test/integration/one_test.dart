library galileo_mysql.test.one_test;
...
List get responseValues {
  ...
  values.add(DateTime(dt.year, dt.month, dt.day).toUtc()); // date has zero'd out time value
  // Datetime has no millis
  values.add(DateTime(dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second).toUtc());
  values.add(DateTime(dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second).toUtc());
...
void main() {
  ...
  test('small blobs', () async {
    ...
    var results = await conn.query('insert into test1 (atext) values (?)', [Blob.fromString(longstring)]);
  ...
  test('medium blobs', () async {
    ...
    var query = await conn.query('insert into test1 (atext) values (?)', [Blob.fromString(longstring)]);
  ...
  test('insert stuff', () async {
    ...
    await conn.query('update test1 set atinyint = ?, adecimal = ?', [127, '123456789.987654321']);
  ...
  test('blobs in prepared queries', () async {
    ...
    var results = await conn.query('insert into test1 (aint, atext) values (?, ?)', [12344, abc]);
    results = await conn.query('select atext from test1 where aint = 12344', []);
    var list = await results.toList();
    expect(list.length, equals(1));
    var values = list[0];
  ...
  test('blobs with nulls', () async {
    await conn.query('insert into test1 (aint, atext) values (12345, \'ABC\u0000DEF\')');
    var results = (await conn.query('select atext from test1 where aint = 12345')).toList();
    expect(results.length, equals(1));
    var v = results.first;
    ...
    await conn.query('insert into test1 (aint, atext) values (?, ?)', [12345, abc]);
    results = (await conn.query('select atext from test1 where aint = 12345', [])).toList();
    expect(results.length, equals(1));
    v = results[0];
    expect(v[0].toString(), equals('ABC\u0000DEF'));
  });
  ...
  test('disallow non-utc datetime serialization', () async {
    expect(() async {
      var results = await conn.query('insert into test1 (adatetime) values (?)', [DateTime.now()]);
      results = await conn.query('select adatetime from test1');
      var dt = results.first[0] as DateTime;
      expect(dt.isUtc, isTrue);
