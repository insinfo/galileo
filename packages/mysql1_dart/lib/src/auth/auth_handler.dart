library galileo_mysql.auth_handler;
...
//  final bool _ssl;
  ...
  AuthHandler(this.username, this.password, this.db, this.scrambleBuffer, this.clientFlags, this.maxPacketSize,
      this.characterSet,
      {bool ssl = false})
      : /*this._ssl = false,*/
  ...
  List<int> getHash() {
    ...
    if (password == null) {
      ...
      final bytes = List<int>.from(scrambleBuffer)..addAll(doubleHashedPassword);
      final hashedSaltedPassword = sha1.convert(bytes).bytes;

      hash = List<int>.generate(
          hashedSaltedPassword.length, (index) => hashedSaltedPassword[index] ^ hashedPassword[index]);
    }
    return hash;
  }
