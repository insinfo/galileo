library galileo_mysql.connection;
...
import 'package:galileo_mysql/src/results/results_impl.dart';
...
class MySqlConnection {
  ...
  static Future<MySqlConnection> connect(ConnectionSettings c, {bool isUnixSocket = false}) async {
    ...
    var socket = await BufferedSocket.connect(c.host, c.port, c.timeout, isUnixSocket: isUnixSocket, onDataReady: () {
      conn?._readPacket();
    }, onDone: () {
      _log.fine('done');
...
class ReqRespConnection {
  ...
  Future<Buffer> _sendBufferPart(Buffer buffer, int start) async {
    ...
    _log.fine('sendBuffer body, buffer length=${buffer.length}, start=$start, len=$len');
