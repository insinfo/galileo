library galileo_isolate.example.http_server;

import 'dart:async';
import 'dart:io';
import 'dart:isolate';

import 'package:galileo_isolate/isolate_runner.dart';
import 'package:galileo_isolate/ports.dart';
import 'package:galileo_isolate/runner.dart';

Future<Future<Object> Function()> runHttpServer(Runner runner, int port, HttpListener listener) async {
...
Future<SendPort> _startHttpServer(List args) async {
  ...
  var server = await HttpServer.bind(InternetAddress.anyIPv6, port, shared: true);
...
void main(List<String> args) async {
  ...
  var socket = await ServerSocket.bind(InternetAddress.anyIPv6, port, shared: true);

  port = socket.port;
  var isolates = await Future.wait(Iterable.generate(5, (_) => IsolateRunner.spawn()), cleanUp: (isolate) {
    isolate.close();
  });
