library galileo_isolate.isolate_runner;
...
class IsolateRunner implements Runner {
  ...
  IsolateRunner(this.isolate, SendPort commandPort) : _commandPort = commandPort;
  ...
  static Future<IsolateRunner> spawn() async {
    ...
    var isolate = await Isolate.spawn(IsolateRunnerRemote._create, channel.port);
  ...
  Future<bool> ping({Duration timeout = const Duration(seconds: 1)}) {
    var channel = SingleResponseChannel(callback: _kTrue, timeout: timeout, timeoutValue: false);
  ...
  /// ```
  @override
  Future<R> run<R, P>(FutureOr<R> Function(P argument) function, P argument, {Duration timeout, Function() onTimeout}) {
