...
library galileo_isolate.load_balancer;
...
class LoadBalancer implements Runner {
  ...
  Future<void> close() {
    ...
    _stopFuture = MultiError.waitUnordered(_queue.take(_length).map((e) => e.close())).then(ignore);
    // Remove all entries.
    for (var i = 0; i < _length; i++) {
...
class _LoadBalancerEntry implements Comparable<_LoadBalancerEntry> {
  ...
  Future<R> run<R, P>(LoadBalancer balancer, int load, FutureOr<R> Function(P argument) function, argument,
