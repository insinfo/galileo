...
import 'package:galileo_pub_sub/json_rpc_2.dart';
...
main() {
  ...
  setUp(() async {
    ...
    adapter = JsonRpc2Adapter(serverSocket.map<StreamChannel<String>>(streamSocket), isTrusted: true);

    var socket1 = await Socket.connect(InternetAddress.loopbackIPv4, serverSocket.port);
    var socket2 = await Socket.connect(InternetAddress.loopbackIPv4, serverSocket.port);
    var socket3 = await Socket.connect(InternetAddress.loopbackIPv4, serverSocket.port);
    var socket4 = await Socket.connect(InternetAddress.loopbackIPv4, serverSocket.port);
    ...
    server = Server([adapter])
      ...
      ..registerClient(const ClientInfo('json_rpc_2_test::no_publish', canPublish: false))
      ..registerClient(const ClientInfo('json_rpc_2_test::no_subscribe', canSubscribe: false))
      ..start();
  ...
  group('json_rpc_2_server', () {
    test('reject unknown client id', () async {
      try {
        var sock = await Socket.connect(InternetAddress.loopbackIPv4, serverSocket.port);
        var client = JsonRpc2Client('json_rpc_2_test::invalid', streamSocket(sock));
    ...
    test('reject unprivileged publish', () async {
      try {
        var sock = await Socket.connect(InternetAddress.loopbackIPv4, serverSocket.port);
        var client = JsonRpc2Client('json_rpc_2_test::no_publish', streamSocket(sock));
        await client.publish('foo', 'bar');
    ...
    test('reject unprivileged subscribe', () async {
      try {
        var sock = await Socket.connect(InternetAddress.loopbackIPv4, serverSocket.port);
        var client = JsonRpc2Client('json_rpc_2_test::no_subscribe', streamSocket(sock));
...
StreamChannel<String> streamSocket(Socket socket) {
  var channel = StreamChannel(socket.cast<List<int>>(), _SocketSink(socket));
  return channel.transform(StreamChannelTransformer.fromCodec(utf8));
