...
import 'package:galileo_pub_sub/isolate.dart';
...
main() {
  ...
  setUp(() async {
    ...
    client1 = IsolateClient('isolate_test::secret', adapter.receivePort.sendPort);
    client2 = IsolateClient('isolate_test::secret2', adapter.receivePort.sendPort);
    client3 = IsolateClient('isolate_test::secret3', adapter.receivePort.sendPort);
    ...
    server = Server([adapter])
      ...
      ..registerClient(const ClientInfo('isolate_test::no_publish', canPublish: false))
      ..registerClient(const ClientInfo('isolate_test::no_subscribe', canSubscribe: false))
      ..start();
  ...
  tearDown(() {
    Future.wait([server.close(), client1.close(), client2.close(), client3.close(), trustedClient.close()]);
  });
  ...
  group('isolate_server', () {
    test('reject unknown client id', () async {
      try {
        var client = new IsolateClient('isolate_test::invalid', adapter.receivePort.sendPort);
    ...
    test('reject unprivileged publish', () async {
      try {
        var client = new IsolateClient('isolate_test::no_publish', adapter.receivePort.sendPort);
        await client.publish('foo', 'bar');
    ...
    test('reject unprivileged subscribe', () async {
      try {
        var client = new IsolateClient('isolate_test::no_subscribe', adapter.receivePort.sendPort);
