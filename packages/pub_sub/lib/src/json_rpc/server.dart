...
class JsonRpc2Adapter extends Adapter {
  ...
  Future close() {
    ...
    Future.wait(_peers.where((s) => !s.isClosed).map((s) => s.close())).then((_) => _peers.clear());
  ...
  void start() {
    clientStream.listen((client) {
      ...
      peer.registerMethod('publish', (json_rpc_2.Parameters params) async {
        ...
        var rq = new _JsonRpc2PublishRequestImpl(requestId, clientId, eventName, value, peer);
        _onPublish.add(rq);
      });
      ...
      peer.registerMethod('subscribe', (json_rpc_2.Parameters params) async {
        ...
        var clientId = _getClientId(params);
        var eventName = params['event_name'].asString;
        var rq = new _JsonRpc2SubscriptionRequestImpl(clientId, eventName, requestId, peer, _uuid);
        _onSubscribe.add(rq);
      });
      ...
      peer.registerMethod('unsubscribe', (json_rpc_2.Parameters params) async {
        ...
        var rq = new _JsonRpc2UnsubscriptionRequestImpl(clientId, subscriptionId, peer, requestId);
...
class _JsonRpc2PublishRequestImpl extends PublishRequest {
  ...
  _JsonRpc2PublishRequestImpl(this.requestId, this.clientId, this.eventName, this.value, this.peer);
  ...
  void reject(String errorMessage) {
    peer.sendNotification(requestId, {'status': false, 'request_id': requestId, 'error_message': errorMessage});
  }
}
...
class _JsonRpc2SubscriptionRequestImpl extends SubscriptionRequest {
  ...
  _JsonRpc2SubscriptionRequestImpl(this.clientId, this.eventName, this.requestId, this.peer, this._uuid);

  @override
  FutureOr<Subscription> accept(String clientId) {
    var id = _uuid.v4();
    peer.sendNotification(
        requestId, {'status': true, 'request_id': requestId, 'subscription_id': id, 'client_id': clientId});
...
class _JsonRpc2UnsubscriptionRequestImpl extends UnsubscriptionRequest {
  ...
  _JsonRpc2UnsubscriptionRequestImpl(this.clientId, this.subscriptionId, this.peer, this.requestId);
