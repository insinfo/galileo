...
class IsolateClient extends Client {
      ...
      if (data is Map && data['request_id'] is String) {
        ...
        if (c != null && !c.isCompleted) {
          if (data['status'] is! bool) {
            c.completeError(new FormatException('The server sent an invalid response.'));
          } else if (!(data['status'] as bool)) {
            c.completeError(new PubSubException(data['error_message']?.toString() ??
                'The server sent a failure response, but did not provide an error message.'));
          } else if (data['result'] is! Map) {
            c.completeError(new FormatException('The server sent a success response, but did not include a result.'));
          } else {
  ...
  Future publish(String eventName, value) {
    return _whenConnected(() {
      ...
      serverSendPort.send({
        ...
        'method': 'publish',
        'params': {'client_id': clientId, 'event_name': eventName, 'value': value}
      });
  ...
  Future<ClientSubscription> subscribe(String eventName) {
    return _whenConnected<ClientSubscription>(() {
      ...
      return c.future.then<ClientSubscription>((result) {
        _clientId = result['client_id'] as String;
        var s = new _IsolateClientSubscription(eventName, result['subscription_id'] as String, this);
  ...
  Future close() {
    ...
    for (var c in _onConnect) {
      if (!c.isCompleted) {
        c.completeError(new StateError('The client was closed before the server ever accepted the connection.'));
      }
    }

    for (var c in _requests.values) {
      if (!c.isCompleted) {
        c.completeError(new StateError('The client was closed before the server responded to this request.'));
      }
    }
