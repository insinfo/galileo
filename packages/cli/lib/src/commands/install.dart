...
import 'package:galileo_mustache4dart/galileo_mustache4dart.dart' as mustache;
...
class InstallCommand extends Command {
  ...
  @override
  String get description => 'Installs additional add-ons to minimize boilerplate.';
  ...
  @override
  run() async {
    print(yellow.wrap('WARNING: The `install` command is no longer considered necessary, and has been deprecated.\n'
        'Expect it to be removed in an upcoming release.\n\n'
        'See here: https://github.com/insinfo/galileo/tree/master/packages/install\n\n'
        'To stop seeing this, downgrade to `package:galileo_cli@<=2.0.0`.'));
    ...
    if (argResults['wipe'] as bool) {
      ...
      if (!await installRepo.exists()) throw 'No local add-on database exists. Run `galileo install --update` first.';

      var pubspec = await loadPubspec();

      for (var packageName in argResults.rest) {
        var packageDir = Directory.fromUri(installRepo.uri.resolve(packageName));

        if (!await packageDir.exists())
          throw 'No add-on named "$packageName" is installed. You might need to run `galileo install --update`.';
        ...
        var deps = projectPubspec.dependencies.keys
            .map((k) {
              ...
              if (dep is HostedDependency) return MakerDependency(k, dep.version.toString());
              return null;
            })
            .where((d) => d != null)
            .toList();
        ...
        deps.addAll(projectPubspec.devDependencies.keys.map((k) {
          ...
          if (dep is HostedDependency) return MakerDependency(k, dep.version.toString(), dev: true);
          return null;
        }).where((d) => d != null));

        await depend(deps);

        var promptFile = File.fromUri(packageDir.uri.resolve('galileo_cli.yaml'));
        ...
        if (await promptFile.exists()) {
          ...
          if (cfg['templates'] is List) {
            globs.addAll((cfg['templates'] as List).map((p) => Glob(p.toString())));
          }
        ...
        Future merge(Directory src, Directory dst, String prefix) async {
          ...
          await for (var entity in src.list()) {
            if (entity is Directory) {
              ...
              await merge(entity, newDir, prefix.isEmpty ? name : '$prefix/$name');
            } else if (entity is File && !entity.path.endsWith('galileo_cli.yaml')) {
              var name = p.basename(entity.path);
              ...
              if (allClear) {
                try {
                  ...
                  if (globs.any((g) => g.matches(path))) {
                    print('Rendering Mustache template from ${entity.absolute.path} to ${targetFile.absolute.path}...');
                    var contents = await entity.readAsString();

                    ///TODO: remover dependencia de mustache
                    var renderer = mustache.compile(contents);
                    var generated = renderer(values);
                    await targetFile.writeAsString(generated.toString());
                  } else {
                    print('Copying ${entity.absolute.path} to ${targetFile.absolute.path}...');
        ...
        await merge(Directory.fromUri(packageDir.uri.resolve('files')), Directory.current, '');
  ...
  Future<List<Pubspec>> list() async {
    if (!await installRepo.exists()) {
      throw 'No local add-on database exists. Run `galileo install --update` first.';
    } else {
      List<Pubspec> repos = [];
