...
class ControllerCommand extends Command {
  ...
  ControllerCommand() {
    argParser
      ..addFlag('websocket',
          abbr: 'w', help: 'Generates a WebSocketController, instead of an HTTP controller.', negatable: false)
      ..addOption('name', abbr: 'n', help: 'Specifies a name for the model class.')
      ..addOption('output-dir',
          help: 'Specifies a directory to create the controller class in.', defaultsTo: 'lib/src/routes/controllers');
  }
  ...
  run() async {
    ...
    var controllerLib = new Library((controllerLib) {
      if (argResults['websocket'] as bool) {
        deps.add(const MakerDependency('galileo_websocket', '^3.0.2'));
        controllerLib.directives.add(new Directive.import('package:galileo_websocket/server.dart'));
      } else {
        controllerLib.directives.add(new Directive.import('package:galileo_framework/galileo_framework.dart'));
      }

      controllerLib.body.add(new Class((clazz) {
        clazz
          ..name = '${rc.pascalCase}Controller'
          ..extend = refer(argResults['websocket'] as bool ? 'WebSocketController' : 'Controller');

        if (argResults['websocket'] as bool) {
          // XController(galileoWebSocket ws) : super(ws);
          clazz.constructors.add(new Constructor((b) {
            b
              ..requiredParameters.add(new Parameter((b) => b
                ..name = 'ws'
                ..type = refer('galileoWebSocket')))
          ...
          clazz.methods.add(new Method((meth) {
            meth
              ..name = 'hello'
              ..returns = refer('void')
              ..annotations.add(refer('ExposeWs').call([literal('get_${rc.snakeCase}')]))
              ..requiredParameters.add(new Parameter((b) => b
                ..name = 'socket'
          ...
          clazz
            ..annotations.add(refer('Expose').call([literal('/${rc.snakeCase}')]))
    ...
    var controllerFile = new File.fromUri(outputDir.uri.resolve('${rc.snakeCase}.dart'));
    if (!await controllerFile.exists()) await controllerFile.create(recursive: true);
    await controllerFile.writeAsString(new DartFormatter().format(controllerLib.accept(new DartEmitter()).toString()));

    print(green.wrap('$checkmark Created controller file "${controllerFile.absolute.path}"'));
