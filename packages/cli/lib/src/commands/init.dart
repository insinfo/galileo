...
class InitCommand extends Command {
  ...
  InitCommand() {
    argParser
      ..addFlag('offline', help: 'Disable online fetching of boilerplates. Also disables `pub-get`.', negatable: false)
      ..addFlag('pub-get', defaultsTo: true)
      ..addOption('project-name', abbr: 'n', help: 'The name for this project.');
  }
  ...
  run() async {
    ...
    await _key.changeSecret(File.fromUri(projectDir.uri.resolve('config/default.yaml')), secret);

    secret = rs.randomAlphaNumeric(32);
    print('Generated new production JWT secret: $secret');
    await _key.changeSecret(File.fromUri(projectDir.uri.resolve('config/production.yaml')), secret);
  ...
  _cloneRepo(Directory projectDir) async {
    ...
    try {
      if (await projectDir.exists()) {
        var shouldDelete = prompts.getBool("Directory '${projectDir.absolute.path}' already exists. Overwrite it?");
      ...
      if (!await boilerplateDir.exists()) {
        ...
        print('Cloning "${boilerplate.name}" boilerplate from "${boilerplate.url}"...');
        Process git;

        if (boilerplate.ref == null) {
          print(darkGray.wrap('\$ git clone --depth 1 ${boilerplate.url} ${boilerplateDir.absolute.path}'));
          git = await Process.start(
            "git",
            ["clone", "--depth", "1", boilerplate.url, boilerplateDir.absolute.path],
      ...
      // Otherwise, pull from git.
        ...
        var git = await Process.start("git", ['pull', 'origin', '$branch'],
            mode: ProcessStartMode.inheritStdio, workingDirectory: boilerplateDir.absolute.path);
        if (await git.exitCode != 0) {
          print(yellow.wrap("Update of $branch failed. Attempting to continue with existing contents."));
        }
      } else {
        print(darkGray.wrap('Using existing contents of "${boilerplate.name}" boilerplate.'));
      }
  ...
  _pubGet(Directory projectDir) async {
    ...
    var pub = await Process.start(pubPath, ["get"],
        workingDirectory: projectDir.absolute.path, mode: ProcessStartMode.inheritStdio);
...
Future preBuild(Directory projectDir) async {
  ...
  var build = await Process.start(resolvePub(), ['run', 'build_runner', 'build'],
      workingDirectory: projectDir.absolute.path, mode: ProcessStartMode.inheritStdio);
...
const BoilerplateInfo sharedBoilerplate = const BoilerplateInfo('Shared',
    'Holds common models and files shared across multiple Dart projects.', '${RepoLocation}/boilerplate_shared.git');
...
class BoilerplateInfo {
  ...
  const BoilerplateInfo(this.name, this.description, this.url, {this.ref, this.needsPrebuild: false});
