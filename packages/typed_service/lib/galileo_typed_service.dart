...
/// An Galileo service that uses reflection to (de)serialize Dart objects.
class TypedService<Id, T> extends Service<Id, T> {
  ...
  TypedService(this.inner) : super() {
    if (!reflectType(T).isAssignableTo(reflectType(Model)))
      throw Exception("If you specify a type for TypedService, it must extend Model.");
  }

  @override
  FutureOr<T> Function(RequestContext, ResponseContext) get readData => _readData;

  T _readData(RequestContext req, ResponseContext res) => deserialize(req.bodyAsMap);
  ...
  @override
  Future<T> create(data, [Map<String, dynamic> params]) => inner.create(serialize(data), params).then(deserialize);

  @override
  Future<T> read(Id id, [Map<String, dynamic> params]) => inner.read(id, params).then(deserialize);
  ...
  @override
  Future<T> remove(Id id, [Map<String, dynamic> params]) => inner.remove(id, params).then(deserialize);
}
