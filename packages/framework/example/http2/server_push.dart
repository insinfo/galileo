...
main() async {
  ...
  var indexHtml = const LocalFileSystem().file(publicDir.uri.resolve('index.html'));
  var styleCss = const LocalFileSystem().file(publicDir.uri.resolve('style.css'));
  var appJs = const LocalFileSystem().file(publicDir.uri.resolve('app.js'));

  // Send files when requested
  app..get('/style.css', (req, res) => res.streamFile(styleCss))..get('/app.js', (req, res) => res.streamFile(appJs));
