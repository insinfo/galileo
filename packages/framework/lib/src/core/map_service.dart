...
class MapService extends Service<String, Map<String, dynamic>> {
  ...
  String get createdAtKey => autoSnakeCaseNames == false ? 'createdAt' : 'created_at';

  String get updatedAtKey => autoSnakeCaseNames == false ? 'updatedAt' : 'updated_at';
  ...
  Future<Map<String, dynamic>> read(String id, [Map<String, dynamic> params]) {
    return Future.value(items.firstWhere(_matchesId(id),
        orElse: () => throw GalileoHttpException.notFound(message: 'No record found for ID $id')));
  }

  @override
  Future<Map<String, dynamic>> create(Map<String, dynamic> data, [Map<String, dynamic> params]) {
    if (data is! Map) {
      throw GalileoHttpException.badRequest(message: 'MapService does not support `create` with ${data.runtimeType}.');
    }
  ...
  Future<Map<String, dynamic>> modify(String id, Map<String, dynamic> data, [Map<String, dynamic> params]) {
    if (data is! Map) {
      throw GalileoHttpException.badRequest(message: 'MapService does not support `modify` with ${data.runtimeType}.');
    }
    ...
    return read(id).then((item) {
      ...
      if (autoIdAndDateFields == true) {
        result..[autoSnakeCaseNames == false ? 'updatedAt' : 'updated_at'] = DateTime.now().toIso8601String();
      }
  ...
  Future<Map<String, dynamic>> update(String id, Map<String, dynamic> data, [Map<String, dynamic> params]) {
    if (data is! Map) {
      throw GalileoHttpException.badRequest(message: 'MapService does not support `update` with ${data.runtimeType}.');
    }
    if (!items.any(_matchesId(id))) return create(data, params);

    return read(id).then((old) {
      if (!items.remove(old)) {
        throw GalileoHttpException.notFound(message: 'No record found for ID $id');
      }
      ...
      if (autoIdAndDateFields == true) {
        result
          ...
          ..[autoSnakeCaseNames == false ? 'updatedAt' : 'updated_at'] = DateTime.now().toIso8601String();
      }
      items.add(result);
      return Future.value(result);
    });
  ...
  Future<Map<String, dynamic>> remove(String id, [Map<String, dynamic> params]) {
    if (id == null || id == 'null') {
      // Remove everything...
      if (!(allowRemoveAll == true || params?.containsKey('provider') != true)) {
        throw GalileoHttpException.forbidden(message: 'Clients are not allowed to delete all items.');
      } else {
        items.clear();
        return Future.value({});
      }
