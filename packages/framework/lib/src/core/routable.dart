library galileo_framework.http.routable;
  ...
  return (req, res) {
    ...
    for (var handler in handlers) {
      ...
      if (runPipeline == null) {
        ...
        runPipeline = () =>
            current().then((result) => !res.isOpen ? Future.value(result) : req.app.executeHandler(handler, req, res));
      }
    }
...
class Routable extends Router<RequestHandler> {
  ...
  T findService<T extends Service>(Pattern path) {
    return _serviceLookups.putIfAbsent(path, () {
      return _services[path] ?? _services[path.toString().replaceAll(_straySlashes, '')];
    }) as T;
  }
  ...
  HookedService<dynamic, dynamic, T> findHookedService<T extends Service>(Pattern path) {
  ...
  Route<RequestHandler> addRoute(String method, String path, RequestHandler handler,
    ...
    if (reflector != null && reflector is! ThrowingReflector) {
      Middleware middlewareDeclaration = getAnnotation<Middleware>(handler, _container?.reflector);
    ...
    return super.addRoute(method, path.toString(), handler, middleware: handlerSequence);
  }
  ...
  HookedService<Id, Data, T> use<Id, Data, T extends Service<Id, Data>>(String path, T service) {
    ...
    _services[path.toString().trim().replaceAll(RegExp(r'(^/+)|(/+$)'), '')] = hooked;
    hooked.addRoutes();
