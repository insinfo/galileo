library galileo_framework.http.request_context;

import 'dart:async';
import 'dart:convert';
import 'dart:io' show BytesBuilder, Cookie, HeaderValue, HttpHeaders, HttpSession, InternetAddress;
...
import 'server.dart' show Galileo;

part 'injection.dart';

/// A convenience wrapper around an incoming [RawRequest].
abstract class RequestContext<RawRequest> {
  /// Similar to [Galileo.shutdownHooks], allows for logic to be executed
  ...
  /// The [Galileo] instance that is responding to this request.
  Galileo app;
  ...
  MediaType get contentType => _contentType ??= MediaType.parse(headers.contentType.toString());
  ...
  bool get isXhr {
    return headers.value("X-Requested-With")?.trim()?.toLowerCase() == 'xmlhttprequest';
  }
  ...
  ///
  /// Use this if the format of the body is not natively parsed by Galileo.
  ...
  set bodyAsObject(value) {
    if (_bodyObject != null) {
      throw StateError('The request body has already been parsed/set, and cannot be overwritten.');
    } else {
  ...
  Map<String, dynamic> get queryParameters => _queryParameters ??= Map<String, dynamic>.from(uri.queryParameters);
  ...
  bool accepts(contentType, {bool strict = false}) {
    var contentTypeString = contentType is MediaType ? contentType.mimeType : contentType?.toString();

    // Change to assert
    if (contentTypeString == null) {
      throw ArgumentError('RequestContext.accepts expects the `contentType` parameter to NOT be null.');
    }
  ...
  Future<T> deserializeBody<T>(FutureOr<T> Function(Map) f, {Encoding encoding = utf8}) async {
  ...
  Future<void> parseBody({Encoding encoding = utf8}) async {
    ...
    if (!_hasParsedBody) {
      ...
      if (contentType.type == 'application' && contentType.subtype == 'json') {
        ...
        var parsed = _bodyObject = await encoding.decoder.bind(body).join().then(json.decode);
      ...
      } else if (contentType.type == 'application' && contentType.subtype == 'x-www-form-urlencoded') {
        _uploadedFiles = [];
        var parsed = await encoding.decoder.bind(body).join().then((s) => Uri.splitQueryString(s, encoding: encoding));
        _bodyFields = Map<String, dynamic>.from(parsed);
        ...
        var parts = transformer.bind(body).map((part) => HttpMultipartFormData.parse(part, defaultEncoding: encoding));
        _bodyFields = {};
        _uploadedFiles = [];
        ...
        await for (var part in parts) {
          ...
          } else if (part.isText && part.contentDisposition.parameters.containsKey('name')) {
...
class UploadedFile {
  ...
  MediaType get contentType =>
      _contentType ??= (formData.contentType == null ? null : MediaType.parse(formData.contentType.toString()));
  ...
  Future<List<int>> readAsBytes() {
    return data.fold<BytesBuilder>(BytesBuilder(), (bb, out) => bb..add(out)).then((bb) => bb.takeBytes());
  }
