part of galileo_framework.http.request_context;
...
resolveInjection(
    requirement, InjectionRequest injection, RequestContext req, ResponseContext res, bool throwOnUnresolved,
  ...
  if (requirement == RequestContext) {
    ...
    return res;
  } else if (requirement is String && injection.parameters.containsKey(requirement)) {
    ...
    if (req.params.containsKey(requirement)) {
      ...
      throw ArgumentError("Cannot resolve parameter '$requirement' within handler.");
    }
    ...
    if (req.params.containsKey(key) || req.app.configuration.containsKey(key) || _primitiveTypes.contains(type)) {
      return await resolveInjection(key, injection, req, res, throwOnUnresolved, container);
    } else {
      return await resolveInjection(type, injection, req, res, throwOnUnresolved, container);
    }
    ...
    throw ArgumentError('$requirement cannot be injected into a request handler.');
  }
}
...
RequestHandler handleContained(Function handler, InjectionRequest injection, [Container container]) {
  return (RequestContext req, ResponseContext res) async {
    ...
    for (var entry in injection.named.entries) {
      ...
      named[name] = await resolveInjection([entry.key, entry.value], injection, req, res, false, container);
    }
...
class InjectionRequest {
  ...
  const InjectionRequest.constant(
      {this.named = const {}, this.required = const [], this.optional = const [], this.parameters = const {}});
...
InjectionRequest preInject(Function handler, Reflector reflector) {
  ...
  for (var parameter in closureMirror.parameters) {
    ...
    var p = parameter.annotations.firstWhere((m) => m.type.isAssignableTo(_Parameter), orElse: () => null)?.reflectee
        as Parameter;
    //print(p);
    if (p != null) {
