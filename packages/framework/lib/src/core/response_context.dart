library galileo_framework.http.response_context;
...
abstract class ResponseContext<RawResponse> implements StreamConsumer<List<int>>, StreamSink<List<int>>, StringSink {
  ...
  /// The [Galileo] instance that is sending a response.
  Galileo app;
  ...
  /// Returns `true` if the response is still available for processing by Galileo.
  ///
  /// If it is `false`, then Galileo will stop executing handlers, and will only run
  ...
  /// Signals Galileo that the response is being held alive deliberately, and that the framework should not automatically close it.
  ///
  ...
  Future<void> download(File file, {String filename}) async {
    ...
    headers["Content-Disposition"] = 'attachment; filename="${filename ?? file.path}"';
  ...
  Future<void> jsonp(value, {String callbackName = "callback", MediaType contentType}) {
    if (!isOpen) throw closed();
  ...
  Future<void> render(String view, [Map<String, dynamic> data]) {
    ...
    return Future<String>.sync(
            () => app.viewGenerator(view, Map<String, dynamic>.from(renderParams)..addAll(data ?? <String, dynamic>{})))
        .then((content) {
      write(content);
      return close();
    });
  ...
  Future<void> redirect(url, {bool absolute = true, int code = 302}) {
    ...
    headers
      ...
      ..['location'] =
          (url is String || url is Uri) ? url.toString() : app.navigate(url as Iterable, absolute: absolute);
    statusCode = code ?? 302;
    write('''
    <!DOCTYPE html>
    <html>
  ...
  Future<void> redirectToAction(String action, [Map params, int code]) {
    ...
    if (split.length < 2) {
      throw Exception("Controller redirects must take the form of 'Controller@action'. You gave: $action");
    }

    Controller controller = app.controllers[split[0].replaceAll(_straySlashes, '')];
    ...
    if (matched == null) {
      throw Exception("Controller '${split[0]}' does not contain any action named '${split[1]}'");
    }

    final head = controller.findExpose(app.container.reflector).path.toString().replaceAll(_straySlashes, '');
    final tail = matched
  ...
  Future streamFile(File file) async {
    ...
    contentType = mimeType == null ? MediaType('application', 'octet-stream') : MediaType.parse(mimeType);

    if (correspondingRequest.method != 'HEAD') {
      return this.addStream(file.openRead().cast<List<int>>()).then((_) => this.close());
    }
  }
...
class _LockableBytesBuilderImpl implements LockableBytesBuilder {
  ...
  StateError _deny() => StateError('Cannot modified a closed response\'s buffer.');
