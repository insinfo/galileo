library galileo_framework.http.metadata;
...
class Expose {
  ...
  const Expose(this.path, {this.method = "GET", this.middleware = const [], this.as, this.allowNull = const []});

  const Expose.method(this.method, {this.middleware, this.as, this.allowNull = const []}) : path = null;
}
...
class Parameter {
  ...
  const Parameter({this.cookie, this.query, this.header, this.session, this.match, this.defaultValue, this.required});

  /// Returns an error that can be thrown when the parameter is not present.
  get error {
    if (cookie?.isNotEmpty == true) {
      return GalileoHttpException.badRequest(message: 'Missing required cookie "$cookie".');
    }
    if (header?.isNotEmpty == true) {
      return GalileoHttpException.badRequest(message: 'Missing required header "$header".');
    }
    if (query?.isNotEmpty == true) {
      return GalileoHttpException.badRequest(message: 'Missing required query parameter "$query".');
    }
  ...
  getValue(RequestContext req) {
    if (cookie?.isNotEmpty == true) {
      return req.cookies.firstWhere((c) => c.name == cookie)?.value ?? defaultValue;
    }
    if (header?.isNotEmpty == true) {
...
class Header extends Parameter {
  const Header(String header, {match, defaultValue, bool required = true})
      : super(header: header, match: match, defaultValue: defaultValue, required: required);
}
...
class Session extends Parameter {
  const Session(String session, {match, defaultValue, bool required = true})
      : super(session: session, match: match, defaultValue: defaultValue, required: required);
}
...
class Query extends Parameter {
  const Query(String query, {match, defaultValue, bool required = true})
      : super(query: query, match: match, defaultValue: defaultValue, required: required);
}
...
class CookieValue extends Parameter {
  const CookieValue(String cookie, {match, defaultValue, bool required = true})
      : super(cookie: cookie, match: match, defaultValue: defaultValue, required: required);
}
