library galileo_framework.http.controller;
...
class Controller {
  ...
  /// The [Galileo] application powering this controller.
  Galileo get app => _app;
  ...
  Future<String> applyRoutes(Router<RequestHandler> router, Reflector reflector) async {
    ...
    final handlers = <RequestHandler>[]..addAll(exposeDecl.middleware)..addAll(middleware);
    final routeBuilder = _routeBuilder(reflector, instanceMirror, routable, handlers);
  ...
  void Function(ReflectedDeclaration) _routeBuilder(
      Reflector reflector, ReflectedInstance instanceMirror, Routable routable, Iterable<RequestHandler> handlers) {
    return (ReflectedDeclaration decl) {
      ...
      if (methodName != 'toString' &&
        ...
        var reflectedMethod = instanceMirror.getField(methodName).reflectee as Function;
        var middleware = <RequestHandler>[]..addAll(handlers)..addAll(exposeDecl.middleware);
        String name = exposeDecl.as?.isNotEmpty == true ? exposeDecl.as : methodName;
        ...
        if (method.parameters.length == 2 &&
          ...
          routeMappings[name] =
              routable.addRoute(exposeDecl.method, exposeDecl.path, (RequestContext req, ResponseContext res) {
        ...
        if (path == null) {
          ...
          if (methodMatch != null) {
            ...
            var restPath = ReCase(rest.isEmpty ? 'index' : rest).snakeCase.replaceAll(_rgxMultipleUnderscores, '_');
          ...
          // If the name is index, though, add "/".
            ...
            if (method.name == 'index') {
              ...
              parts.add(ReCase(method.name).snakeCase.replaceAll(_rgxMultipleUnderscores, '_'));
            }
          }
        ...
        routeMappings[name] =
            routable.addRoute(httpMethod, path, handleContained(reflectedMethod, injection), middleware: middleware);
      }
    };
  }
