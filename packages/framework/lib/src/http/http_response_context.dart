...
class HttpResponseContext extends ResponseContext<HttpResponse> {
  ...
  set contentType(MediaType value) {
    ...
    if (!_streamInitialized) {
      rawResponse.headers.contentType = ContentType(value.type, value.subtype, parameters: value.parameters);
    }
  }
  ...
  bool _openStream() {
    if (!_streamInitialized) {
      ...
      if (headers.containsKey('content-length')) {
        rawResponse.contentLength = int.tryParse(headers['content-length']) ?? rawResponse.contentLength;
      }

      rawResponse.headers.contentType = ContentType(contentType.type, contentType.subtype,
          charset: contentType.parameters['charset'], parameters: contentType.parameters);
