...
import 'dart:convert';
import 'dart:io';
import 'package:galileo_container/src/container.dart';
...
class Http2RequestContext extends RequestContext<ServerTransportStream> {
  ...
  static Future<Http2RequestContext> from(
      ServerTransportStream stream, Socket socket, Galileo app, Map<String, MockHttpSession> sessions, Uuid uuid) {
    ...
    var uri = Uri(scheme: 'https', host: socket.address.address, port: socket.port);
    var cookies = <Cookie>[];
    ...
    // Apply session
    var dartSessId = cookies.firstWhere((c) => c.name == 'DARTSESSID', orElse: () => null);
