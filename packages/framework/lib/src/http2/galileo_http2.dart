...
import 'dart:convert';
import 'dart:io';
import 'package:galileo_framework/galileo_framework.dart' hide Header;
...
Future<SecureServerSocket> startSharedHttp2(address, int port, SecurityContext ctx) {
  return SecureServerSocket.bind(address, port, ctx, shared: true);
}

/// Adapts `package:http2`'s [ServerTransportConnection] to serve Galileo.
class GalileoHttp2
    extends Driver<Socket, ServerTransportStream, SecureServerSocket, Http2RequestContext, Http2ResponseContext> {
  final ServerSettings settings;
  GalileoHttp _http;
  final StreamController<HttpRequest> _onHttp1 = StreamController();
  final Map<String, MockHttpSession> _sessions = {};
  final Uuid _uuid = Uuid();
  _GalileoHttp2ServerSocket _artificial;

  SecureServerSocket get socket => _artificial;

  GalileoHttp2._(Galileo app, Future<SecureServerSocket> Function(dynamic, int) serverGenerator, bool useZone,
      bool allowHttp1, this.settings)
      : super(
          app,
          serverGenerator,
          useZone: useZone,
    ...
    if (allowHttp1) {
      _http = GalileoHttp(app, useZone: useZone);
      onHttp1.listen(_http.handleRequest);
    }
  }

  factory GalileoHttp2(Galileo app, SecurityContext securityContext,
      {bool useZone = true, bool allowHttp1 = false, ServerSettings settings}) {
    return GalileoHttp2.custom(app, securityContext, SecureServerSocket.bind,
        allowHttp1: allowHttp1, settings: settings);
  }

  factory GalileoHttp2.custom(Galileo app, SecurityContext ctx,
      Future<SecureServerSocket> serverGenerator(address, int port, SecurityContext ctx),
      {bool useZone = true, bool allowHttp1 = false, ServerSettings settings}) {
    return GalileoHttp2._(app, (address, port) {
      var addr = address is InternetAddress ? address : InternetAddress(address.toString());
  ...
  Future<SecureServerSocket> generateServer([address, int port]) async {
    ...
    return _artificial = _GalileoHttp2ServerSocket(s, this);
  }
  ...
  void addCookies(ServerTransportStream response, Iterable<Cookie> cookies) {
    var headers = cookies.map((cookie) => Header.ascii('set-cookie', cookie.toString()));
  ...
  @override
  Future<Http2RequestContext> createRequestContext(Socket request, ServerTransportStream response) {
    return Http2RequestContext.from(response, request, app, _sessions, _uuid);
  }

  @override
  Future<Http2ResponseContext> createResponseContext(Socket request, ServerTransportStream response,
      [Http2RequestContext correspondingRequest]) async {
    return Http2ResponseContext(app, response, correspondingRequest)..encoders.addAll(app.encoders);
  }

  @override
  Stream<ServerTransportStream> createResponseStreamFromRawRequest(Socket request) {
    var connection = ServerTransportConnection.viaSocket(request, settings: settings);
  ...
  @override
  Uri get uri => Uri(scheme: 'https', host: server.address.address, port: server.port != 443 ? server.port : null);
...
class _FakeServerSocket extends Stream<Socket> implements ServerSocket {
  final _GalileoHttp2ServerSocket galileo;
  final _ctrl = StreamController<Socket>();

  _FakeServerSocket(this.galileo);

  @override
  InternetAddress get address => galileo.address;
  ...
  @override
  int get port => galileo.port;

  @override
  StreamSubscription<Socket> listen(void Function(Socket event) onData,
      {Function onError, void Function() onDone, bool cancelOnError}) {
    return _ctrl.stream.listen(onData, cancelOnError: cancelOnError, onError: onError, onDone: onDone);
  }
}

class _GalileoHttp2ServerSocket extends Stream<SecureSocket> implements SecureServerSocket {
  final SecureServerSocket socket;
  final GalileoHttp2 driver;
  final _ctrl = StreamController<SecureSocket>();
  _FakeServerSocket _fake;
  StreamSubscription _sub;

  _GalileoHttp2ServerSocket(this.socket, this.driver) {
    ...
    _sub = socket.listen(
      (socket) {
        ...
        } else if (socket.selectedProtocol == 'h2' || socket.selectedProtocol == 'h2-14') {
          _ctrl.add(socket);
        } else {
          socket.destroy();
          throw Exception('GalileoHttp2 does not support ${socket.selectedProtocol} as an ALPN protocol.');
        }
      },
      onDone: _ctrl.close,
      onError: (e, st) {
        driver.app.logger.warning('HTTP/2 incoming connection failure: ', e, st as StackTrace);
      },
    );
  }
  ...
  @override
  StreamSubscription<SecureSocket> listen(void Function(SecureSocket event) onData,
      {Function onError, void Function() onDone, bool cancelOnError}) {
    return _ctrl.stream.listen(onData, cancelOnError: cancelOnError, onError: onError, onDone: onDone);
  }
}
