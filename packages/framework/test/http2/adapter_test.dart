...
const String jfk = 'Ask not what your country can do for you, but what you can do for your country.';
...
void main() {
  ...
  setUp(() async {
    app = Galileo(reflector: MirrorsReflector())..encoders['gzip'] = gzip.encoder;
    hierarchicalLoggingEnabled = true;
    app.logger = Logger.detached('galileo.http2')
      ..onRecord.listen((rec) {
        print(rec);
    ...
    app.get('/headers', (req, res) async {
      res.headers.addAll({'foo': 'bar', 'x-galileo': 'http2'});
      await res.close();
    });
    ...
    app.post('/upload', (req, res) async {
      ...
      return [await file.data.map((l) => l.length).reduce((a, b) => a + b), file.contentType.mimeType, body];
    });
    ...
    http2 = GalileoHttp2(app, ctx, allowHttp1: true);
  ...
  group('gzip', () {
    test('buffered response', () async {
      var response = await client.get(serverRoot, headers: {'accept-encoding': 'gzip, deflate, br'});
    ...
    test('streamed response', () async {
      var response = await client.get(serverRoot.replace(path: '/stream'), headers: {'accept-encoding': 'gzip'});
      expect(response.headers['content-encoding'], 'gzip');
      //print(response.body);
      var decoded = gzip.decode(response.bodyBytes);
  ...
  test('query uri decoded', () async {
    var uri = serverRoot.replace(path: '/query', queryParameters: {'foo!': 'bar?'});
  ...
  test('headers sent', () async {
    ...
    expect(response.headers['x-galileo'], 'http2');
  });
  ...
  test('server push', () async {
    ...
    String getPath(TransportStreamPush p) =>
        ascii.decode(p.requestHeaders.firstWhere((h) => ascii.decode(h.name) == ':path').value);
  ...
  group('body parsing', () {
    ...
    test('multipart body parsed', () async {
      var rq = http.MultipartRequest('POST', serverRoot.replace(path: '/upload'));
      rq.headers.addAll({'accept': 'application/json'});

      rq.fields['foo'] = 'bar';
      rq.files.add(http.MultipartFile('file', Stream.fromIterable([utf8.encode('hello world')]), 11,
          contentType: MediaType('galileo', 'framework')));
