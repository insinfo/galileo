...
main() {
  ...
  setUp(() async {
    ...
    nested = Galileo(reflector: MirrorsReflector());
    todos = Galileo(reflector: MirrorsReflector());

    [app, nested, todos].forEach((Galileo app) {
    ...
    app.get('/intercepted', (req, res) => 'This should not be shown', middleware: [interceptor]);
    app.get('/hello', (req, res) => 'world');
    app.get('/name/:first/last/:last', (req, res) => req.params);
    app.post('/lambda', (RequestContext req, res) => req.parseBody().then((_) => req.bodyAsMap));
    app.mount('/todos/:id', todos);
    app.get('/greet/:name', (RequestContext req, res) async => "Hello ${req.params['name']}").name = 'Named routes';
    ...
    app.chain([write('a')]).chain([write('b'), write('c')]).get('/chained', (req, res) => res.close());
    ...
    client = http.Client();
    var server = await GalileoHttp(app).startServer('127.0.0.1', 0);
  ...
  test('Can nest another Galileo instance', () async {
    var response = await client.post(Uri.parse('$url/nes/ted/foo'));
    var json_ = json.decode(response.body);
    expect(json_['route'], equals('foo'));
  });

  test('Can parse parameters from a nested Galileo instance', () async {
  ...
  test('Can serialize function result as JSON', () async {
    ...
    var response = await client.post(Uri.parse("$url/lambda"), headers: headers as Map<String, String>, body: postData);
    print('Response: ${response.body}');
  ...
  test('Can name routes', () {
    ...
    print('Foo: $foo');
    String uri = foo.makeUri({'id': 'galileo'});
    print(uri);
    expect(uri, equals('framework/galileo'));
  });
  ...
  test('Match routes, even with query params', () async {
    var response = await client.get(Uri.parse("$url/log?foo=bar&bar=baz&baz.foo=bar&baz.bar=foo"));
    print(response.body);
