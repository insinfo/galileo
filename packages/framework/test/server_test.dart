...
main() {
  group('scoping', () {
    var parent = Galileo(reflector: MirrorsReflector())..configuration['two'] = 2;
    var child = Galileo(reflector: MirrorsReflector());
  ...
  test('custom server generator', () {
    ...
    var http = GalileoHttp.custom(app, HttpServer.bind);
  ...
  test('default error handler', () async {
    ...
    var e = GalileoHttpException(null, statusCode: 321, message: 'Hello', errors: ['foo', 'bar']);
  ...
  test('warning when adding routes to flattened router', () {
    var app = Galileo(reflector: MirrorsReflector())..optimizeForProduction(force: true);
    app.dumpTree();
    app.get('/', (req, res) => 2);
  ...
  test('global injection added to injection map', () async {
    var app = Galileo(reflector: MirrorsReflector())..configuration['a'] = 'b';
    var http = GalileoHttp(app);
  ...
  test('global injected serializer', () async {
    var app = Galileo(reflector: MirrorsReflector())..serializer = (_) => 'x';
    var http = GalileoHttp(app);
  ...
  group('handler results', () {
    var app = Galileo(reflector: MirrorsReflector());
    var http = GalileoHttp(app);
    app.responseFinalizers.add((req, res) => throw GalileoHttpException.forbidden());
    RequestContext req;
    ResponseContext res;
  ...
  group('handleGalileoHttpException', () {
    Galileo app;
    GalileoHttp http;

    setUp(() async {
      app = Galileo(reflector: MirrorsReflector());
      app.get('/wtf', (req, res) => throw GalileoHttpException.forbidden());
      app.get('/wtf2', (req, res) => throw GalileoHttpException.forbidden());
    ...
    test('can send json', () async {
      var rq = MockHttpRequest('GET', Uri(path: 'wtf'))..headers.set('accept', 'application/json');
      (rq.close());
