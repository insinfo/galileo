...
void encodingTests(Galileo getApp()) {
  group('encoding', () {
    Galileo app;
    GalileoHttp http;
    ...
    test('encodes if wildcard', () async {
      var rq = MockHttpRequest('GET', Uri.parse('/hello'))..headers.set('accept-encoding', '*');
      await rq.close();
      var rs = rq.response;
      await http.handleRequest(rq);
    ...
    test('encodes if wildcard + multiple', () async {
      var rq = MockHttpRequest('GET', Uri.parse('/hello'))..headers.set('accept-encoding', ['foo', 'bar', '*']);
      await rq.close();
      var rs = rq.response;
      await http.handleRequest(rq);
    ...
    test('encodes if explicit', () async {
      var rq = MockHttpRequest('GET', Uri.parse('/hello'))..headers.set('accept-encoding', 'gzip');
      await rq.close();
      var rs = rq.response;
      await http.handleRequest(rq);
    ...
    test('only uses one encoder', () async {
      var rq = MockHttpRequest('GET', Uri.parse('/hello'))..headers.set('accept-encoding', ['gzip', 'deflate']);
      await rq.close();
      var rs = rq.response;
      await http.handleRequest(rq);
