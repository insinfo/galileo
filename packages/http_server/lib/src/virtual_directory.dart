...
import 'package:galileo_http_server/src/has_current_iterator.dart';
...
class VirtualDirectory {
  ...
  static List<String> _parsePathPrefix(String? pathPrefix) {
    ...
    return Uri(path: pathPrefix).pathSegments.where((segment) => segment.isNotEmpty).toList();
  }
  ...
  /// generated.
  VirtualDirectory(this.root, {String? pathPrefix}) : _pathPrefixSegments = _parsePathPrefix(pathPrefix);

  /// Serve a [Stream] of [HttpRequest]s, in this [VirtualDirectory].
  StreamSubscription<HttpRequest> serve(Stream<HttpRequest> requests) => requests.listen(serveRequest);
  ...
  Future serveRequest(HttpRequest request) async {
    ...
    if (entity is File) {
      ...
      _unawaited(request.response.redirect(Uri.parse('${request.uri}/'), status: HttpStatus.movedPermanently));
    } else {
      assert(entity == null);
      _serveErrorPage(HttpStatus.notFound, request);
    }
  ...
  Future<Object?> _locateResource(String path, HasCurrentIterator<String> segments) async {
  ...
  void _serveDirectory(Directory dir, HttpRequest request) async {
    ...
    try {
      ...
      if (request.headers.ifModifiedSince != null && !stats.modified.isAfter(request.headers.ifModifiedSince!)) {
        response.statusCode = HttpStatus.notModified;
        await response.close();
        return;
      }

      response.headers.contentType = ContentType('text', 'html', parameters: {'charset': 'utf-8'});
      ...
      var header = '''<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN"
...
''';
      ...
      void add(String name, String? modified, var size, bool folder) {
        ...
        var encodedLink = const HtmlEscape(HtmlEscapeMode.attribute).convert(Uri.encodeComponent(name));
        if (folder) {
          encodedLink += '/';
          name += '/';
        }
