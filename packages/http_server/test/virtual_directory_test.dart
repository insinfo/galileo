...
void main() {
  ...
  group('serve-dir', () {
      ...
      if (!Platform.isWindows) {
        testVirtualDir('recursive-link', (dir) async {
          ...
          var result = await Future.wait([
            ...
            fetchAsString(virDir, '/recursive').then((s) => s.contains('recursive&#47;')),
            fetchAsString(virDir, '/recursive').then((s) => s.contains('..&#47;')),
            fetchAsString(virDir, '/recursive').then((s) => s.contains('Index of &#47;recursive'))
          ]);
    ...
    group('custom', () {
      ...
      testVirtualDir('index-2', (dir) async {
        ...
        var result = await statusCodeForVirtDir(virDir, '/dir', followRedirects: false);
        expect(result, 301);
      });
  ...
  group('last-modified', () {
    group('file', () {
      testVirtualDir('file-exists', (dir) async {
        ...
        var lastModified = HttpDate.parse(headers.value(HttpHeaders.lastModifiedHeader)!);

        var result = await statusCodeForVirtDir(virDir, '/file', ifModifiedSince: lastModified);
  ...
  group('range', () {
    ...
    testVirtualDir('range', (dir) async {
      prepare(dir);
      Future<void> check(int from, int to, [List<int>? expected, String? contentRange]) async {
        expected ??= fileContent.sublist(from, to + 1);
        contentRange ??= 'bytes $from-$to/${fileContent.length}';
        var result = await fetchContentAndResponse(virDir, '/file', from: from, to: to);
        var content = result[0];
        var response = result[1];
        expect(content, expected);
        expect(response.headers[HttpHeaders.contentRangeHeader][0], contentRange);
    ...
    testVirtualDir('suffix-range', (dir) async {
      prepare(dir);
      Future<void> check(int to, [List<int>? expected, String? contentRange]) async {
        expected ??= fileContent.sublist(fileContent.length - to, fileContent.length);
  ...
  group('url-decode', () {
    ...
    testVirtualDir('encoded-path-separator', (dir) async {
      ...
      var result = await statusCodeForVirtDir(virDir, '/a%2fb/c', rawPath: true);
      expect(result, HttpStatus.notFound);
    });
