...
Future _postDataTest(List<int> message, String contentType, String boundary, List<FormField> expectedFields,
  ...
  server.listen((request) async {
    ...
    var fields = await MimeMultipartTransformer(boundary)
      ...
      if (multipart.isText) {
        ...
        data = await multipart.fold<List<int>>([], (b, s) => b..addAll(s as List<int>));
      }
      String? contentType;
      ...
      return FormField(multipart.contentDisposition.parameters['name'], data,
          contentType: contentType, filename: multipart.contentDisposition.parameters['filename']);
    }).toList();
...
------WebKitFormBoundaryU3FBruSkJKG0Yor1--\r\n''';
    ...
    await _postDataTest(message0.codeUnits, 'multipart/form-data', '----WebKitFormBoundaryU3FBruSkJKG0Yor1', []);
  });
...
--AaB03x--\r\n''';
    ...
    await _postDataTest(message.codeUnits, 'multipart/form-data', 'AaB03x', [
      ...
      FormField('files', 'Content of file', contentType: 'text/plain', filename: 'file1.txt')
    ]);
  });
...
--AaB03x--\r\n''';
    ...
    await _postDataTest(message.codeUnits, 'multipart/form-data', 'AaB03x', [FormField('submit-name', 'Larry')]);
  });
...
--AaB03x--\r\n''';
    ...
    await _postDataTest(message.codeUnits, 'multipart/form-data', 'AaB03x',
        [FormField('files', 'Content of file', contentType: 'text/plain', filename: 'C:\\file1".txt')]);
  });
  ...
  test('Similar test using Chrome posting.', () async {
    ...
    await _postDataTest(message2, 'multipart/form-data', '----WebKitFormBoundaryQSql8kDALM7tAkC1', [
      FormField('submit-name', 'Test'),
      FormField('files', data, contentType: 'application/octet-stream', filename: 'VERSION')
    ]);
  });
  ...
  test('HTML entity encoding in values in form fields', () async {
    ...
    await _postDataTest(message3, 'multipart/form-data', '----WebKitFormBoundaryvAVzugKMtZbyWoBG',
        [FormField('name', '&#12402;&#12425;&#12364;&#12394;')],
  ...
  test('UTF', () async {
    ...
    await _postDataTest(
        message4, 'multipart/form-data', '----WebKitFormBoundaryGXtBrcjxheKeN6i0', [FormField('test', 'ひらがな')],
        defaultEncoding: utf8);
  });
  ...
  test('WebKit', () async {
    ...
    await _postDataTest(
        message5, 'multipart/form-data', '----WebKitFormBoundaryfe0EzV1aNysD1bPh', [FormField('name', 'øv')]);
  });
}
