...
void _testVirtualDir(String name, bool useFakes, Future<void> Function(Directory) func) {
  if (useFakes) {
...
Future<int> statusCodeForVirtDir(VirtualDirectory virtualDir, String path,
  ...
  if (_isFakeTestExpando[currentTestCase]!) {
    ...
    var request = FakeHttpRequest(uri,
        followRedirects: followRedirects, ifModifiedSince: ifModifiedSince, data: StreamController<Uint8List>().stream);
...
Future<HttpHeaders> fetchHEaders(VirtualDirectory virDir, String path, {int? from, int? to}) async {
  // if this is a fake test, then run the fake code path
  if (_isFakeTestExpando[currentTestCase]!) {
    var uri = _localhostUri(0, path);

    var request = FakeHttpRequest(uri, data: StreamController<Uint8List>().stream);
    _addRangeHeader(request, from, to);
...
Future<List<int>> fetchAsBytes(VirtualDirectory virtualDir, String path, {int? from, int? to}) async {
  // if this is a fake test, then run the fake code path
  if (_isFakeTestExpando[currentTestCase]!) {
    var uri = _localhostUri(0, path);
  ...
  return _withServer(virtualDir, (int port) => _fetchAsBytes(port, path, from, to));
}

Future<List> fetchContentAndResponse(VirtualDirectory virtualDir, String path, {int? from, int? to}) async {
  // if this is a fake test, then run the fake code path
  if (_isFakeTestExpando[currentTestCase]!) {
    var uri = _localhostUri(0, path);
  ...
  return _withServer(virtualDir, (int port) => _fetchContentAndResponse(port, path, from, to));
}

Future<FakeHttpResponse> _withFakeRequest(VirtualDirectory virDir, FakeHttpRequest request) async {
  ...
  if (response.statusCode == HttpStatus.movedPermanently || response.statusCode == HttpStatus.movedTemporarily) {
    if (request.followRedirects == true) {
      var uri = Uri.parse(response.headers.value(HttpHeaders.locationHeader)!);
      var newMock = FakeHttpRequest(uri, followRedirects: true, data: StreamController<Uint8List>().stream);
...
Future<T> _withServer<T>(VirtualDirectory virDir, Future<T> Function(int port) func) async {
...
Future<List> _fetchContentAndResponse(int port, String path, int? from, int? to) async {
  var client = HttpClient();
  try {
    var request = await client.get('localhost', port, path);
    _addRangeHeader(request, from, to);
...
Uri _localhostUri(int port, String path, {bool secure = false, bool rawPath = false}) {
  if (rawPath) {
    return Uri(scheme: secure ? 'https' : 'http', host: 'localhost', port: port, path: path);
  } else {
    return (secure ? Uri.https('localhost:$port', path) : Uri.http('localhost:$port', path));
  }
}
...
void setupSecure() {
  var currentFileUri = (reflect(setupSecure) as ClosureMirror).function.location!.sourceUri;

  String localFile(String path) => currentFileUri.resolve(path).toFilePath();

  serverContext = SecurityContext()
    ..useCertificateChain(localFile('certificates/server_chain.pem'))
    ..usePrivateKey(localFile('certificates/server_key.pem'), password: 'dartdart');

  clientContext = SecurityContext()..setTrustedCertificates(localFile('certificates/trusted_certs.pem'));
}
