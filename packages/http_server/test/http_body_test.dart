...
void _testHttpClientResponseBody() {
  void check(String mimeType, List<int> content, dynamic expectedBody, String type, [bool shouldFail = false]) async {
...
void _testHttpServerRequestBody() {
  void check(String? mimeType, List<int> content, dynamic expectedBody, String type,
...
void main() {
  ...
  test('Does not close stream while requests are pending', () async {
    ...
    var requests = Stream<HttpRequest>.fromIterable([FakeHttpRequest(Uri(), data: data.stream)]);
    var isDone = false;
