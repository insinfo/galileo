part of galileo_postgres.connection;
...
class _PostgreSQLConnectionStateSocketConnected extends _PostgreSQLConnectionState {
  ...
  _PostgreSQLConnectionState onEnter() {
    final startupMessage =
        StartupMessage(connection!.databaseName, connection!.timeZone, username: connection!.username);
  ...
  _PostgreSQLConnectionState onMessage(ServerMessage message) {
    ...
    completer
        .completeError(PostgreSQLException('Unsupported authentication type ${authMessage.type}, closing connection.'));
...
class _PostgreSQLConnectionStateAuthenticating extends _PostgreSQLConnectionState {
  ...
  @override
  _PostgreSQLConnectionState onEnter() {
    final authMessage = AuthMD5Message(connection!.username!, connection!.password!, connection!._salt);
  ...
  _PostgreSQLConnectionState onMessage(ServerMessage message) {
    ...
    if (message is ReadyForQueryMessage) {
      if (message.state == ReadyForQueryMessage.StateTransactionError) {
        ...
        return _PostgreSQLConnectionStateReadyInTransaction(query.transaction as _TransactionProxy);
      }
      ...
      final validationException = query.validateParameters(message.parameterTypeIDs);
