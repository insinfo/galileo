library galileo_postgres.connection;
...
class PostgreSQLConnection extends Object with _PostgreSQLExecutionContextMixin implements PostgreSQLExecutionContext {
  ...
  final StreamController<Notification> _notifications = StreamController<Notification>.broadcast();
  ...
  Future open() async {
    if (_hasConnectedPreviously) {
      throw PostgreSQLException('Attempting to reopen a closed connection. Create a instance instead.');
    }

    try {
      _hasConnectedPreviously = true;
      if (isUnixSocket) {
        _socket = await Socket.connect(InternetAddress(host, type: InternetAddressType.unix), port)
            .timeout(Duration(seconds: timeoutInSeconds));
      } else {
        _socket = await Socket.connect(host, port).timeout(Duration(seconds: timeoutInSeconds));
      }
      ...
      _transitionToState(_PostgreSQLConnectionStateSocketConnected(connectionComplete));

      await connectionComplete.future.timeout(Duration(seconds: timeoutInSeconds));
    } on TimeoutException catch (e, st) {
      final err = PostgreSQLException('Failed to connect to database $host:$port/$databaseName failed to connect.');
      await _close(err, st);
      rethrow;
    } catch (e, st) {
      await _close(e, st);
  ...
  Future transaction(
    ...
    if (isClosed) {
      throw PostgreSQLException('Attempting to execute query, but connection is not open.');
    }
    ...
    while (_framer.hasMessage) {
      ...
      try {
        if (msg is ErrorResponseMessage) {
          ...
          _notifications.add(Notification(msg.processID, msg.channel, msg.payload));
        } else {
  ...
  Future<Socket> _upgradeSocketToSSL(Socket originalSocket, {int timeout = 30}) {
    ...
    originalSocket.listen((data) {
      if (data.length != 1) {
        sslCompleter
            .completeError(PostgreSQLException('Could not initalize SSL connection, received unknown byte stream.'));
        return;
      }
      ...
      sslCompleter.complete(data.first);
        ...
        onDone: () => sslCompleter.completeError(
            PostgreSQLException('Could not initialize SSL connection, connection closed during handshake.')),
    ...
    return sslCompleter.future.timeout(Duration(seconds: timeout)).then((responseByte) {
      if (responseByte != 83) {
        throw PostgreSQLException('The database server is not accepting SSL connections.');
      }

      return SecureSocket.secure(originalSocket, onBadCertificate: (certificate) => true)
...
class _OidCache {
  ...
  Future<List<FieldDescription>> _resolveTableNames(
      _PostgreSQLExecutionContextMixin c, List<FieldDescription?>? columns) async {
    ...
    return columns.map((c) => c!.change(tableName: _tableOIDNameMap[c.tableID])).toList();
  }

  Future _resolveTableOIDs(_PostgreSQLExecutionContextMixin c, List<int> oids) async {
    _queryCount++;
...
abstract class _PostgreSQLExecutionContextMixin implements PostgreSQLExecutionContext {
  ...
  Future<PostgreSQLResult> _query(
    ...
    final query = Query<List<List<dynamic>>>(fmtString, substitutionValues, _connection, _transaction);
    if (allowReuse) {
    ...
    if (resolveOids) {
      columnDescriptions = await _connection._oidCache._resolveTableNames(this, columnDescriptions);
    }
    final metaData = _PostgreSQLResultMetaData(columnDescriptions!);

    return _PostgreSQLResult(queryResult.affectedRowCount, metaData,
        queryResult.value!.map((columns) => _PostgreSQLResultRow(metaData, columns)).toList());
  }

  @override
  Future<List<Map<String, Map<String, dynamic>>>> mappedResultsQuery(String fmtString,
      {Map<String, dynamic>? substitutionValues = const {}, bool? allowReuse = false, int? timeoutInSeconds}) async {
  ...
  Future<int> execute(String fmtString,
      {Map<String, dynamic>? substitutionValues = const {}, int? timeoutInSeconds}) async {
    timeoutInSeconds ??= _connection.queryTimeoutInSeconds;
    if (_connection.isClosed) {
      throw PostgreSQLException('Attempting to execute query, but connection is not open.');
    }
    ...
    final query =
        Query<void>(fmtString, substitutionValues, _connection, _transaction, onlyReturnAffectedRowCount: true);
  ...
  Future<QueryResult<T>> _enqueue<T>(Query<T> query, {int timeoutInSeconds = 30}) async {
    if (_queue.add(query)) {
      ...
      try {
        final result = await query.future.timeout(Duration(seconds: timeoutInSeconds));
...
class _PostgreSQLResultMetaData {
  ...
  _PostgreSQLResultMetaData(this.columnDescriptions) {
    _tableNames = columnDescriptions.map((column) => column.tableName).toSet().toList();
  }
...
class _PostgreSQLResult extends UnmodifiableListView<PostgreSQLResultRow> implements PostgreSQLResult {
  @override
  final int affectedRowCount;
  final _PostgreSQLResultMetaData _metaData;

  _PostgreSQLResult(this.affectedRowCount, this._metaData, List<PostgreSQLResultRow> rows) : super(rows);

  @override
  List<ColumnDescription> get columnDescriptions => _metaData.columnDescriptions;
}

class _PostgreSQLResultRow extends UnmodifiableListView implements PostgreSQLResultRow {
  final _PostgreSQLResultMetaData _metaData;
