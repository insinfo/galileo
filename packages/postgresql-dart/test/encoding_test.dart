...
import 'package:galileo_postgres/src/binary_codec.dart';
import 'package:galileo_postgres/src/text_codec.dart';
import 'package:galileo_postgres/src/types.dart';
import 'package:galileo_postgres/src/utf8_backed_string.dart';
...
void main() {
  group('Binary encoders', () {
    setUp(() async {
      conn = PostgreSQLConnection('localhost', dbPort, 'dart_test', username: 'dart', password: 'dart');
      await conn.open();
    });
    ...
    test('timestamp', () async {
      await expectInverse(DateTime.utc(1920, 10, 1), PostgreSQLDataType.timestampWithoutTimezone);
      await expectInverse(DateTime.utc(2120, 10, 5), PostgreSQLDataType.timestampWithoutTimezone);
      try {
    ...
    test('timestamptz', () async {
      await expectInverse(DateTime.utc(1920, 10, 1), PostgreSQLDataType.timestampWithTimezone);
      await expectInverse(DateTime.utc(2120, 10, 5), PostgreSQLDataType.timestampWithTimezone);
      try {
    ...
    test('jsonb', () async {
      ...
      try {
        await conn.query('INSERT INTO t (v) VALUES (@v:jsonb)', substitutionValues: {'v': DateTime.now()});
        fail('unreachable');
    ...
    test('bytea', () async {
      ...
      try {
        await conn.query('INSERT INTO t (v) VALUES (@v:bytea)', substitutionValues: {'v': DateTime.now()});
        fail('unreachable');
      } on FormatException catch (e) {
    ...
    test('uuid', () async {
      ...
      try {
        await conn.query('INSERT INTO t (v) VALUES (@v:uuid)', substitutionValues: {'v': DateTime.now()});
        fail('unreachable');
      } on FormatException catch (e) {
        expect(e.toString(), contains('Expected: String'));
      }
    ...
    test('varchar', () async {
      ...
      try {
        await conn.query('INSERT INTO t (v) VALUES (@v:varchar)', substitutionValues: {'v': 0});
        fail('unreachable');
      } on FormatException catch (e) {
        expect(e.toString(), contains('Expected: String'));
      }
    ...
    test('json', () async {
      ...
      try {
        await conn.query('INSERT INTO t (v) VALUES (@v:json)', substitutionValues: {'v': DateTime.now()});
        fail('unreachable');
      } on JsonUnsupportedObjectError catch (_) {}
    });
    ...
    test('point', () async {
      ...
      try {
        await conn.query('INSERT INTO t (v) VALUES (@v:point)', substitutionValues: {'v': 'text'});
        fail('unreachable');
      } on FormatException catch (e) {
    ...
    test('integerArray', () async {
      ...
      try {
        await conn.query('INSERT INTO t (v) VALUES (@v:_int4)', substitutionValues: {'v': 'not-list-int'});
        fail('unreachable');
      } on FormatException catch (e) {
        expect(e.toString(), contains('Expected: List<int>'));
      }
    ...
    test('doubleArray', () async {
      ...
      try {
        await conn.query('INSERT INTO t (v) VALUES (@v:_float8)', substitutionValues: {'v': 'not-list-double'});
        fail('unreachable');
      } on FormatException catch (e) {
    ...
    test('textArray', () async {
      ...
      try {
        await conn.query('INSERT INTO t (v) VALUES (@v:_text)', substitutionValues: {'v': 0});
        fail('unreachable');
      } on FormatException catch (e) {
    ...
    test('jsonbArray', () async {
      ...
      try {
        await conn.query('INSERT INTO t (v) VALUES (@v:_jsonb)', substitutionValues: {
          'v': [DateTime.now()]
        });
        fail('unreachable');
      } on JsonUnsupportedObjectError catch (_) {}
  ...
  group('Text encoders', () {
    ...
    test('Encode DateTime', () {
      ...
      final pairs = {
        '2001-02-03T00:00:00.000$tzOffsetDelimiter': DateTime(2001, DateTime.february, 3),
        '2001-02-03T04:05:06.000$tzOffsetDelimiter': DateTime(2001, DateTime.february, 3, 4, 5, 6, 0),
        '2001-02-03T04:05:06.999$tzOffsetDelimiter': DateTime(2001, DateTime.february, 3, 4, 5, 6, 999),
        '0010-02-03T04:05:06.123$tzOffsetDelimiter BC': DateTime(-10, DateTime.february, 3, 4, 5, 6, 123),
        '0010-02-03T04:05:06.000$tzOffsetDelimiter BC': DateTime(-10, DateTime.february, 3, 4, 5, 6, 0),
        '012345-02-03T04:05:06.000$tzOffsetDelimiter BC': DateTime(-12345, DateTime.february, 3, 4, 5, 6, 0),
        '012345-02-03T04:05:06.000$tzOffsetDelimiter': DateTime(12345, DateTime.february, 3, 4, 5, 6, 0)
      };
...
Future expectInverse(dynamic value, PostgreSQLDataType dataType) async {
  ...
  final result = await conn.query('INSERT INTO t (v) VALUES (${PostgreSQLFormat.id('v', type: dataType)}) RETURNING v',
