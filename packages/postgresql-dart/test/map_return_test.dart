...
void main() {
  ...
  setUp(() async {
    ...
    await connection.execute("INSERT INTO u (id, name, t_id) VALUES (1, 'ua', 1)");
    await connection.execute("INSERT INTO u (id, name, t_id) VALUES (2, 'ub', 1)");
    await connection.execute("INSERT INTO u (id, name, t_id) VALUES (3, 'uc', 2)");
  });
  ...
  test('Get row map without specifying columns', () async {
    final results = await connection.mappedResultsQuery('SELECT * from t ORDER BY id ASC');
    expect(results, [
      {
        't': {'id': 1, 'name': 'a'}
      },
  ...
  test('Get row map by with specified columns', () async {
    final results = await connection.mappedResultsQuery('SELECT name, id from t ORDER BY id ASC');
    expect(results, [
      {
        't': {'id': 1, 'name': 'a'}
      },
    ...
    final nextResults = await connection.mappedResultsQuery('SELECT name from t ORDER BY name DESC');
    expect(nextResults, [
      {
        't': {'name': 'c'}
      },
  ...
  test('Table names get cached', () async {
    ...
    await connection.mappedResultsQuery('SELECT t.id, u.id FROM t LEFT OUTER JOIN u ON t.id=u.t_id');
...
void clearOidQueryCount(PostgreSQLConnection connection) {
  final oidCacheMirror = reflect(connection)
      ...
      .declarations
      .values
      .firstWhere((DeclarationMirror dm) => dm.simpleName.toString().contains('_oidCache'));
...
int getOidQueryCount(PostgreSQLConnection connection) {
  final oidCacheMirror = reflect(connection)
      ...
      .values
      .firstWhere((DeclarationMirror dm) => dm.simpleName.toString().contains('_oidCache'));
  return (reflect(connection).getField(oidCacheMirror.simpleName).reflectee).queryCount as int;
}
