...
void main() {
  ...
  group('Storage', () {
    ...
    test('Can store JSON String with driver type annotation', () async {
      var result =
          await connection.query('INSERT INTO t (j) VALUES (@a:jsonb) RETURNING j', substitutionValues: {'a': 'xyz'});
      expect(result, [
        ['xyz']
      ]);
      result = await connection.query('SELECT j FROM t');
    ...
    test('Can store JSON Number with driver type annotation', () async {
      var result =
          await connection.query('INSERT INTO t (j) VALUES (@a:jsonb) RETURNING j', substitutionValues: {'a': 4});
      expect(result, [
        [4]
      ]);
      result = await connection.query('SELECT j FROM t');
