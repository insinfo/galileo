...
import 'package:galileo_postgres/src/query.dart';
import 'package:test/test.dart';

void main() {
  test('Ensure all types/format type mappings are available and accurate', () {
    PostgreSQLDataType.values
        .where((t) => t != PostgreSQLDataType.bigSerial && t != PostgreSQLDataType.serial)
        .forEach((t) {
  ...
  test('Two identifiers next to eachother', () {
    final result = PostgreSQLFormat.substitute('@id@bob', {'id': 20, 'bob': 13});
  ...
  test('Identifiers next to eachother with type info', () {
    final result = PostgreSQLFormat.substitute('@id:int2@foo:float4', {'id': 12, 'foo': 2.0});
  ...
  test('Can both provide type and typecast', () {
    final results =
        PostgreSQLFormat.substitute('SELECT * FROM t WHERE id=@id:int2::int4', {'id': 2, 'blob': '{"key":"value"}'});
  ...
  test('UTF16 symbols with quotes', () {
    final value = "'©™®'";
    final results = PostgreSQLFormat.substitute('INSERT INTO t (t) VALUES (@t)', {'t': value});
  ...
  test('String identifiers get escaped', () {
    final result = PostgreSQLFormat.substitute('@id:text @foo', {'id': "1';select", 'foo': '3\\4'});

    //                         '  1  '  '  ;  s   e   l   e   c  t   '  sp  sp  E  '  3  \  \  4  '
    expect(
        utf8.encode(result), [39, 49, 39, 39, 59, 115, 101, 108, 101, 99, 116, 39, 32, 32, 69, 39, 51, 92, 92, 52, 39]);
  });
