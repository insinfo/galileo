...
void main() {
  ...
  group('Connection lifecycle', () {
    ...
    test('SSL Connect with md5 auth required', () async {
      conn = PostgreSQLConnection('localhost', dbPort, 'dart_test', username: 'dart', password: 'dart', useSSL: true);

      await conn.open();

      expect(await conn.execute('select 1'), equals(1));
      final socketMirror = reflect(conn)
          .type
          .declarations
          .values
          .firstWhere((DeclarationMirror dm) => dm.simpleName.toString().contains('_socket'));
      final underlyingSocket = reflect(conn).getField(socketMirror.simpleName).reflectee;
      expect(underlyingSocket is SecureSocket, true);
    });

    test('Connect with no auth required', () async {
      conn = PostgreSQLConnection('localhost', dbPort, 'dart_test', username: 'darttrust');
      await conn.open();
    ...
    test('SSL Connect with no auth required', () async {
      conn = PostgreSQLConnection('localhost', dbPort, 'dart_test', username: 'darttrust', useSSL: true);
      await conn.open();
    ...
    test('Closing idle connection succeeds, closes underlying socket', () async {
      ...
      final socketMirror = reflect(conn)
          ...
          .values
          .firstWhere((DeclarationMirror dm) => dm.simpleName.toString().contains('_socket'));
      final underlyingSocket = reflect(conn).getField(socketMirror.simpleName).reflectee as Socket;
  ...
  group('Unintended user-error situations', () {
    ...
    test('Invalid password reports error, conn is closed, disables conn', () async {
      conn = PostgreSQLConnection('localhost', dbPort, 'dart_test', username: 'dart', password: 'notdart');
    ...
    test('SSL Invalid password reports error, conn is closed, disables conn', () async {
      conn =
          PostgreSQLConnection('localhost', dbPort, 'dart_test', username: 'dart', password: 'notdart', useSSL: true);
    ...
    test('A query error maintains connectivity, continues processing pending queries', () async {
      ...
      final queueMirror = reflect(conn)
          .type
          .instanceMembers
          .values
          .firstWhere((DeclarationMirror dm) => dm.simpleName.toString().contains('_queue'));
      final queue = reflect(conn).getField(queueMirror.simpleName).reflectee as List;
  ...
  group('Network error situations', () {
    ...
    test('SSL Socket fails to connect reports error, disables connection for future use', () async {
      final conn = PostgreSQLConnection('localhost', 5431, 'dart_test', useSSL: true);
    ...
    test('Connection that times out throws appropriate error and cannot be reused', () async {
      ...
      final conn = PostgreSQLConnection('localhost', 5433, 'dart_test', timeoutInSeconds: 2);
    ...
    test('SSL Connection that times out throws appropriate error and cannot be reused', () async {
      ...
      final conn = PostgreSQLConnection('localhost', 5433, 'dart_test', timeoutInSeconds: 2, useSSL: true);
  ...
  test('If connection is closed, do not allow .execute', () async {
    final conn = PostgreSQLConnection('localhost', dbPort, 'dart_test', username: 'dart', password: 'dart');
    try {
      await conn.execute('SELECT 1');
      fail('unreachable');
    } on PostgreSQLException catch (e) {
