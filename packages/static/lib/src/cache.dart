...
class CachingVirtualDirectory extends VirtualDirectory {
  ...
  CachingVirtualDirectory(Galileo app, FileSystem fileSystem,
  ...
  @override
  Future<bool> serveFile(File file, FileStat stat, RequestContext req, ResponseContext res) {
    ...
    if (!shouldNotCache) {
      shouldNotCache = req.headers.value('cache-control') == 'no-cache' || req.headers.value('pragma') == 'no-cache';
    }
      ...
      if (ifModified != null) {
        try {
          ...
          throw GalileoHttpException.badRequest(
              message: 'Invalid date for ${ifRange ? 'if-range' : 'if-not-modified-since'} header.');
        }
      }
        ...
        if (etagsToMatchAgainst?.isNotEmpty == true) {
          ...
          for (var etag in etagsToMatchAgainst) {
            if (etag == '*') {
              ...
              hasBeenModified = !_etags.containsKey(file.absolute.path) || _etags[file.absolute.path] != etag;
            }
          }
  ...
  void setCachedHeaders(DateTime modified, RequestContext req, ResponseContext res) {
