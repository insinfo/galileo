...
class VirtualDirectory {
  ...
  Future<bool> servePath(String path, RequestContext req, ResponseContext res) async {
    if (_prefix.isNotEmpty) {
      ...
      // Resolve: https://github.com/Galileo-dart/Galileo/issues/41
  ...
  Future<bool> serveStat(
      String absolute, String relative, FileStat stat, RequestContext req, ResponseContext res) async {
    if (stat.type == FileSystemEntityType.directory) {
      return await serveDirectory(fileSystem.directory(absolute), relative, stat, req, res);
  ...
  Future<bool> serveDirectory(
      Directory directory, String relative, FileStat stat, RequestContext req, ResponseContext res) async {
    for (var indexFileName in indexFileNames) {
      final index = fileSystem.file(directory.absolute.uri.resolve(indexFileName));
    ...
    if (allowDirectoryListing == true) {
      ...
      res
        ..write('<!DOCTYPE html>')
        ..write('<html>')
        ..write('<head><meta name="viewport" content="width=device-width,initial-scale=1">')
        ..write('<style>ul { list-style-type: none; }</style>')
        ..write('</head><body>');

      res.write('<li><a href="..">..</a></li>');

      var entities = await directory.list(followLinks: false).toList().then((l) => List.from(l));
      entities.sort((a, b) {
        if (a is Directory) {
          if (b is Directory) {
  ...
  void _ensureContentTypeAllowed(String mimeType, RequestContext req) {
    ...
    var acceptable = value == null ||
        ...
        value?.contains('*/*') == true;
    if (!acceptable) {
      throw GalileoHttpException(UnsupportedError('Client requested $value, but server wanted to send $mimeType.'),
  ...
  Future<bool> serveFile(File file, FileStat stat, RequestContext req, ResponseContext res) async {
    ...
    if (callback != null) {
      return await req.app
          .executeHandler((RequestContext req, ResponseContext res) => callback(file, req, res), req, res);
    }

    var type = app.mimeTypeResolver.lookup(file.path) ?? 'application/octet-stream';
    res.headers['accept-ranges'] = 'bytes';
    ...
    if (req.headers.value('range')?.startsWith('bytes=') != true) {
      ...
      for (var item in header.items) {
        ...
        if (invalid) {
          throw GalileoHttpException(Exception('Semantically invalid, or unbounded range.'),
              statusCode: 416, message: 'Semantically invalid, or unbounded range.');
        }

        // Ensure it's within range.
        if (item.start >= totalFileSize || item.end >= totalFileSize) {
          throw GalileoHttpException(Exception('Given range $item is out of bounds.'),
              statusCode: 416, message: 'Given range $item is out of bounds.');
        }
      }

      if (header.items.isEmpty) {
        throw GalileoHttpException(null, statusCode: 416, message: '`Range` header may not be empty.');
        ...
        res.statusCode = 206;
        res.headers['content-length'] = transformer.computeContentLength(totalFileSize).toString();
