library galileo_serialize_generator;
...
Builder jsonModelBuilder(_) {
  return SharedPartBuilder(const [JsonModelGenerator()], 'galileo_serialize');
}

Builder serializerBuilder(_) {
  return SharedPartBuilder(const [SerializerGenerator()], 'galileo_serialize_serializer');
}
...
Expression convertObject(DartObject o) {
  ...
  if (o.toMapValue() != null) {
    return literalMap(o.toMapValue().map((k, v) => MapEntry(convertObject(k), convertObject(v))));
  }
  ...
  return target.call(
      rev.positionalArguments.map(convertObject), rev.namedArguments.map((k, v) => MapEntry(k, convertObject(v))));
}
...
/// Determines if a type supports `package:galileo_serialize`.
