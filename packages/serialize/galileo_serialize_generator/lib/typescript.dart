...
class TypeScriptDefinitionBuilder implements Builder {
  ...
  Future<String> compileToTypeScriptType(BuildContext ctx, String fieldName, DartType type, List<String> refs,
    ...
    if (type is InterfaceType) {
      if (isListOfModelType(type)) {
        var arg = await compileToTypeScriptType(ctx, fieldName, type.typeArguments[0], refs, ext, buildStep);
        typeScriptType = '$arg[]';
      } else if (const TypeChecker.fromRuntime(Map).isAssignableFromType(type) && type.typeArguments.length == 2) {
        var key = await compileToTypeScriptType(ctx, fieldName, type.typeArguments[0], refs, ext, buildStep);
        var value = await compileToTypeScriptType(ctx, fieldName, type.typeArguments[1], refs, ext, buildStep);
        ...
        typeScriptType = ctx.modelClassNameRecase.pascalCase + ReCase(fieldName).pascalCase;
        ...
        if (!p.equals(sourcePath, targetPath)) {
          ...
          if (type.element.source.uri.scheme == 'asset') {
            var id = AssetId.resolve(type.element.source.uri);
          ...
          if (ref == null) {
            ...
            var filename = p.setExtension(p.basenameWithoutExtension(relative), '.d.ts');
        ...
        var ctx = await buildContext(
          type.element,
          ConstantReader(serializableTypeChecker.firstAnnotationOf(type.element)),
          buildStep,
          buildStep.resolver,
          autoSnakeCaseNames,
        );
  ...
  Future build(BuildStep buildStep) async {
    ...
    try {
      elements = lib.annotatedWith(const TypeChecker.fromRuntime(Serializable)).toList();
    } catch (_) {
    ...
    for (var element in elements) {
      ...
      contexts.add(await buildContext(element.element as ClassElement, element.annotation, buildStep,
    ...
    for (var ctx in contexts) {
      ...
      for (var field in ctx.fields) {
        ...
        var typeScriptType = await compileToTypeScriptType(
            ctx, field.name, ctx.resolveSerializedFieldType(field.name), refs, ext, buildStep);
