part of galileo_serialize_generator;
...
class JsonModelGenerator extends GeneratorForAnnotation<Serializable> {
  ...
  Future<String> generateForAnnotatedElement(Element element, ConstantReader annotation, BuildStep buildStep) async {
    ...
    var ctx = await buildContext(element as ClassElement, annotation, buildStep, await buildStep.resolver, true);
  ...
  void generateClass(BuildContext ctx, LibraryBuilder file, ConstantReader annotation) {
  ...
  bool shouldBeConstant(BuildContext ctx) {
    ...
    return !isAssignableToModel(ctx.clazz.thisType) &&
        ctx.clazz.fields.every((f) => f.getter?.isAbstract != false && f.setter?.isAbstract != false);
  }

  /// Generate a constructor with named parameters.
  void generateConstructor(BuildContext ctx, ClassBuilder clazz, LibraryBuilder file) {
    clazz.constructors.add(Constructor((constructor) {
      // Add all `super` params
      constructor.constant = (ctx.clazz.unnamedConstructor?.isConst == true || shouldBeConstant(ctx)) &&
          ctx.fields.every((f) {
      ...
      for (var field in ctx.fields) {
        if (!shouldBeConstant(ctx) && isListOrMapType(field.type)) {
          var typeName = const TypeChecker.fromRuntime(List).isAssignableFromType(field.type) ? 'List' : 'Map';
      ...
      for (var field in ctx.fields) {
        constructor.optionalParameters.add(Parameter((b) {
          ...
          if (ctx.requiredFields.containsKey(field.name) && b.defaultTo == null) {
      ...
      if (ctx.constructorParameters.isNotEmpty) {
        if (!shouldBeConstant(ctx) || ctx.clazz.unnamedConstructor?.isConst == true) {
          constructor.initializers.add(Code('super(${ctx.constructorParameters.map((p) => p.name).join(',')})'));
        }
      }
    }));
  }
  ...
  void generateCopyWithMethod(BuildContext ctx, ClassBuilder clazz, LibraryBuilder file) {
    clazz.methods.add(Method((method) {
      method
        ..name = 'copyWith'
    ...
    if (type is InterfaceType) {
      if (const TypeChecker.fromRuntime(List).isAssignableFromType(type)) {
        ...
        if (type.typeArguments.length == 2) {
          var keq = generateEquality(type.typeArguments[0]), veq = generateEquality(type.typeArguments[1]);
    ...
    clazz
      ..methods.add(Method((method) {
        method
          ...
          ..body = refer('hashObjects').call([literalList(ctx.fields.map((f) => refer(f.name)))]).returned.statement;
      }));
  }
  ...
  void generateEqualsOperator(BuildContext ctx, ClassBuilder clazz, LibraryBuilder file) {
    clazz.methods.add(Method((method) {
      method
        ..name = 'operator =='
