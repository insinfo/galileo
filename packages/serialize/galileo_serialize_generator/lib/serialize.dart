...
class SerializerGenerator extends GeneratorForAnnotation<Serializable> {
  ...
  Future<String> generateForAnnotatedElement(Element element, ConstantReader annotation, BuildStep buildStep) async {
    ...
    var ctx = await buildContext(
        element as ClassElement, annotation, buildStep, await buildStep.resolver, autoSnakeCaseNames != false);
  ...
  void generateClass(List<int> serializers, BuildContext ctx, LibraryBuilder file) {
    // Generate canonical codecs, etc.
    var pascal = ctx.modelClassNameRecase.pascalCase, camel = ctx.modelClassNameRecase.camelCase;
...
class ${pascal}Decoder extends Converter<Map, ${pascal}> {
  ...
  ${pascal} convert(Map map) => ${pascal}Serializer.fromMap(map);
    ...
    file.body.add(Class((clazz) {
      ...
      if (serializers.contains(Serializers.map) || serializers.contains(Serializers.json)) {
  ...
  void generateFromMapMethod(ClassBuilder clazz, BuildContext ctx, LibraryBuilder file) {
    clazz.methods.add(Method((method) {
      ...
      for (var field in ctx.fields) {
        ...
        String deserializedRepresentation = "map['$alias'] as ${typeToString(type)}";
        ...
        if (ctx.fieldInfo[field.name]?.deserializer != null) {
          var name = MirrorSystem.getName(ctx.fieldInfo[field.name].deserializer);
        ...
        // Serialize model classes via `XSerializer.toMap`
          ...
          if (isListOfModelType(type)) {
            ...
            ''';
          } else if (const TypeChecker.fromRuntime(List).isAssignableFromType(type) && type.typeArguments.length == 1) {
            var arg = convertTypeReference(type.typeArguments[0]).accept(DartEmitter());
            deserializedRepresentation = '''
            ...
            var key = convertTypeReference(type.typeArguments[0]).accept(DartEmitter());
            var value = convertTypeReference(type.typeArguments[1]).accept(DartEmitter());
            deserializedRepresentation = '''
                map['$alias'] is Map
    ...
    file.body.add(Class((clazz) {
      ...
      clazz.fields.add(Field((b) {
        b
          ...
          ..name = 'allFields'
          ..assignment = literalConstList(ctx.fields.map((f) => refer(f.name)).toList(), refer('String')).code;
      }));
