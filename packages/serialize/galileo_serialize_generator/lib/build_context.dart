...
const TypeChecker serializableFieldTypeChecker = TypeChecker.fromRuntime(SerializableField);

const TypeChecker serializableTypeChecker = TypeChecker.fromRuntime(Serializable);

const TypeChecker generatedSerializableTypeChecker = TypeChecker.fromRuntime(GeneratedSerializable);

final Map<String, BuildContext> _cache = {};

/// Create a [BuildContext].
Future<BuildContext> buildContext(
    ClassElement clazz, ConstantReader annotation, BuildStep buildStep, Resolver resolver, bool autoSnakeCaseNames,
  ...
  var ctx = BuildContext(
    ...
    includeAnnotations: annotation.peek('includeAnnotations')?.listValue ?? <DartObject>[],
  );
  ...
  for (var field in fields) {
    ...
    if (field.getter != null && (field.setter != null || field.getter.isAbstract)) {
      ...
      if (fieldAnn != null) {
        ...
        // Check for @required
        var required = const TypeChecker.fromRuntime(Required).firstAnnotationOf(el);
