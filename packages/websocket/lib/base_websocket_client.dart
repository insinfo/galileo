...
/// An [Galileo] client that operates across WebSockets.
abstract class BaseWebSocketClient extends BaseGalileoClient {
  ...
  final StreamController<WebSocketEvent> _onAllEvents = StreamController<WebSocketEvent>();
  final StreamController<GalileoAuthResult> _onAuthenticated = StreamController<GalileoAuthResult>();
  final StreamController<GalileoHttpException> _onError = StreamController<GalileoHttpException>();
  final StreamController<Map<String, WebSocketEvent>> _onServiceEvent =
      StreamController<Map<String, WebSocketEvent>>.broadcast();
  final StreamController<WebSocketChannelException> _onWebSocketChannelException =
  ...
  /// Fired on errors.
  Stream<GalileoHttpException> get onError => _onError.stream;

  /// Fired whenever an event is fired by a service.
  Stream<Map<String, WebSocketEvent>> get onServiceEvent => _onServiceEvent.stream;

  /// Fired on [WebSocketChannelException]s.
  Stream<WebSocketChannelException> get onWebSocketChannelException => _onWebSocketChannelException.stream;
  ...
  BaseWebSocketClient(http.BaseClient client, baseUrl, {this.reconnectOnClose = true, Duration reconnectInterval})
    ...
    if (timeout != null) {
      ...
      timer = Timer(timeout, () {
        if (!c.isCompleted) {
          ...
          c.completeError(
              TimeoutException('WebSocket connection exceeded timeout of ${timeout.inMilliseconds} ms', timeout));
        }
      });
  ...
  @override
  WebSocketsService<Id, Data> service<Id, Data>(String path, {Type type, GalileoDeserializer<Data> deserializer}) {
    String uri = path.toString().replaceAll(_straySlashes, '');
    return WebSocketsService<Id, Data>(socket, this, uri, deserializer: deserializer);
  }
          ...
          if (data is WebSocketChannelException) {
            ...
            if (jsons is Map) {
              ...
              if (event.eventName == errorEvent) {
                var error = GalileoHttpException.fromMap((event.data ?? {}) as Map);
                _onError.add(error);
              } else if (event.eventName == authenticatedEvent) {
                var authResult = GalileoAuthResult.fromMap(event.data as Map);
                _onAuthenticated.add(authResult);
              } else if (event.eventName?.isNotEmpty == true) {
                var split = event.eventName.split("::").where((str) => str.isNotEmpty).toList();
...
class WebSocketsService<Id, Data> extends Service<Id, Data> {
  ...
  WebSocketEvent<Data> transformEvent(WebSocketEvent event) {
    return WebSocketEvent(eventName: event.eventName, data: deserialize(event.data));
  }
    ...
    app.onServiceEvent.listen((map) {
      if (map.containsKey(path)) {
        ...
        if (event.eventName == indexedEvent) {
          var d = event.data;
          var transformed =
              WebSocketEvent(eventName: event.eventName, data: d is Iterable ? d.map(deserialize).toList() : null);
  ...
  Future<List<Data>> index([Map<String, dynamic> params]) async {
    app.sendAction(WebSocketAction(eventName: '$path::$indexAction', params: params ?? {}));
    return null;
  }

  @override
  Future<Data> read(id, [Map<String, dynamic> params]) async {
    app.sendAction(WebSocketAction(eventName: '$path::$readAction', id: id.toString(), params: params ?? {}));
    return null;
  }

  @override
  Future<Data> create(data, [Map<String, dynamic> params]) async {
    app.sendAction(WebSocketAction(eventName: '$path::$createAction', data: data, params: params ?? {}));
    return null;
  }
  ...
  Future<Data> modify(id, data, [Map<String, dynamic> params]) async {
    app.sendAction(
        WebSocketAction(eventName: '$path::$modifyAction', id: id.toString(), data: data, params: params ?? {}));
    return null;
  }
  ...
  Future<Data> update(id, data, [Map<String, dynamic> params]) async {
    app.sendAction(
        WebSocketAction(eventName: '$path::$updateAction', id: id.toString(), data: data, params: params ?? {}));
    return null;
  }

  @override
  Future<Data> remove(id, [Map<String, dynamic> params]) async {
    app.sendAction(WebSocketAction(eventName: '$path::$removeAction', id: id.toString(), params: params ?? {}));
    return null;
  }
...
class WebSocketExtraneousEventHandler {
  ...
  StreamController<WebSocketEvent> _getStream(String index) {
    if (_events[index] == null) _events[index] = StreamController<WebSocketEvent>();
