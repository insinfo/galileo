...
library galileo_client;

import 'dart:async';
import 'package:collection/collection.dart';
import 'dart:convert';
import 'package:http/http.dart' as http;
export 'package:galileo_http_exception/galileo_http_exception.dart';
import 'package:meta/meta.dart';

/// A function that configures an [Galileo] client in some way.
...
/// doesn't work.
typedef T GalileoDeserializer<T>(x);

/// Represents an Galileo server that we are querying.
abstract class Galileo extends http.BaseClient {
  /// A mutable member. When this is set, it holds a JSON Web Token
  /// that is automatically attached to every request sent.
  ///
  /// This is designed with `package:galileo_auth` in mind.
  String authToken;
  ...
  Stream<GalileoAuthResult> get onAuthenticated;
  ...
  Future<GalileoAuthResult> reviveJwt(String token, {String authEndpoint = '/auth'}) {
    return authenticate(type: 'token', credentials: {'token': token}, authEndpoint: authEndpoint);
  }
  ...
  Future<void> configure(GalileoConfigurer configurer) async {
  ...
  ///
  /// This expects that there is an Galileo `Service` mounted on the server.
  ///
  ...
  Service<Id, Data> service<Id, Data>(String path, {@deprecated Type type, GalileoDeserializer<Data> deserializer});
  ...
  @override
  Future<http.Response> patch(url, {body, Map<String, String> headers, Encoding encoding});

  @override
  Future<http.Response> post(url, {body, Map<String, String> headers, Encoding encoding});

  @override
  Future<http.Response> put(url, {body, Map<String, String> headers, Encoding encoding});
}

/// Represents the result of authentication with an Galileo server.
class GalileoAuthResult {
  String _token;
  ...
  GalileoAuthResult({String token, Map<String, dynamic> data = const {}}) {
    _token = token;
    this.data.addAll(data ?? {});
  }

  /// Attempts to deserialize a response from a [Map].
  factory GalileoAuthResult.fromMap(Map data) {
    final result = GalileoAuthResult();

    if (data is Map && data.containsKey('token') && data['token'] is String) result._token = data['token'].toString();

    if (data is Map) result.data.addAll((data['data'] as Map<String, dynamic>) ?? {});

    if (result.token == null) {
      throw FormatException('The required "token" field was not present in the given data.');
  ...
  factory GalileoAuthResult.fromJson(String s) => GalileoAuthResult.fromMap(json.decode(s) as Map);
...
/// Queries a service on an Galileo server, with the same API.
abstract class Service<Id, Data> {
  ...
  /// The Galileo instance powering this service.
  Galileo get app;
...
class _MappedService<Id, Data, U> extends Service<Id, U> {
  ...
  @override
  Stream<List<U>> get onIndexed => inner.onIndexed.map((l) => l.map(encoder).toList());
...
class ServiceList<Id, Data> extends DelegatingList<Data> {
  ...
  ServiceList(this.service, {this.idField = 'id', Equality<Data> equality}) : super([]) {
    _equality = equality;
