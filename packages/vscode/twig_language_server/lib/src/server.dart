...
class TwigLanguageServer extends LanguageServer {
  ...
  void setupExtraMethods(json_rpc_2.Peer peer) {
    peer.registerMethod('textDocument/formatting', (json_rpc_2.Parameters params) async {
      var documentId = new TextDocumentIdentifier.fromJson(params['textDocument'].asMap);
      var formattingOptions = new FormattingOptions.fromJson(params['options'].asMap);
  ...
  Future<ServerCapabilities> initialize(
      int clientPid, String rootUri, ClientCapabilities clientCapabilities, String trace) async {
    ...
    // Copy all real files that end in *.twig (and *.tw for legacy) into the in-memory filesystem.
      ...
      b
        ...
        ..completionProvider = new CompletionOptions((b) {
          b
            ...
            ..triggerCharacters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdeghijklmnopqrstuvxwyz'
                .codeUnits
  ...
  Future textDocumentDidOpen(TextDocumentItem document) async {
    await analyzerForId(new TextDocumentIdentifier((b) => b..uri = document.uri));
  }

  @override
  Future textDocumentDidChange(
      VersionedTextDocumentIdentifier documentId, List<TextDocumentContentChangeEvent> changes) async {
    ...
    for (var change in changes) {
      if (change.text != null) {
        ...
        int findIndex(Position position) {
          ...
          int lineLength = lines.take(position.line - 1).map((s) => s.length).reduce((a, b) => a + b);
        ...
        if (change.range == null) {
          ...
          var start = findIndex(change.range.start), end = findIndex(change.range.end);
  ...
  @override
  Future<List> textDocumentCodeAction(TextDocumentIdentifier documentId, Range range, CodeActionContext context) async {
    // TODO: implement textDocumentCodeAction
    return [];
  }

  @override
  Future<CompletionList> textDocumentCompletion(TextDocumentIdentifier documentId, Position position) async {
  ...
  Future<String> currentName(TextDocumentIdentifier documentId, Position position) async {
    // First, read the file.
    var file = await fileForId(documentId);
    var contents = await file.readAsString();
    ...
    while (!scanner.isDone && (scanner.state.line != position.line || scanner.state.column != position.character)) {
      scanner.readChar();
    }
  ...
  Future<TwigObject> currentSymbol(TextDocumentIdentifier documentId, Position position) async {
    ...
    logger.info('Current symbols, seeking $name: ${symbols.map((v) => v.name)}');
    return analyzer.scope.resolve(name)?.value;
  }

  @override
  Future<Location> textDocumentDefinition(TextDocumentIdentifier documentId, Position position) async {
  ...
  @override
  Future<List<DocumentHighlight>> textDocumentHighlight(TextDocumentIdentifier documentId, Position position) async {
    var symbol = await currentSymbol(documentId, position);
    if (symbol != null) {
      return symbol.usages.map((u) {
        return new DocumentHighlight((b) {
          b
            ...
            ..kind = u.type == SymbolUsageType.definition ? DocumentHighlightKind.write : DocumentHighlightKind.read;
        });
      }).toList();
    }
    return [];
  ...
  @override
  Future<Hover> textDocumentHover(TextDocumentIdentifier documentId, Position position) async {
    var symbol = await currentSymbol(documentId, position);
    if (symbol != null) {
      return new Hover((b) {
        b
  ...
  @override
  Future<List<Location>> textDocumentImplementation(TextDocumentIdentifier documentId, Position position) async {
  ...
  Future<List<Location>> textDocumentReferences(
      TextDocumentIdentifier documentId, Position position, ReferenceContext context) async {
    var symbol = await currentSymbol(documentId, position);
    if (symbol != null) {
      return symbol.usages.map((u) {
  ...
  @override
  Future<WorkspaceEdit> textDocumentRename(TextDocumentIdentifier documentId, Position position, String newName) async {
    var symbol = await currentSymbol(documentId, position);
    if (symbol != null) {
  ...
  @override
  Future<List<SymbolInformation>> textDocumentSymbols(TextDocumentIdentifier documentId) async {
    var analyzer = await analyzerForId(documentId);
  ...
  Future<List<TextEdit>> textDocumentFormatting(
    ...
    try {
      ...
      var formatter = fmt.TwigFormatter(formattingOptions.tabSize, formattingOptions.insertSpaces);
...
class FormattingOptions {
  ...
  factory FormattingOptions.fromJson(Map json) {
    return new FormattingOptions(json['tabSize'] as num, json['insertSpaces'] as bool);
  }
}
