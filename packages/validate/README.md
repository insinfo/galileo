...
Validation library based on the `matcher` library, with galileo support.
...
* [Use with galileo](#use-with-galileo)
...
If these are not present, `galileo_validate` will *attempt* to generate
...
* `validate(validator)`: Validates and filters `req.bodyAsMap`, and throws an `galileoHttpException.BadRequest` if data is invalid.
