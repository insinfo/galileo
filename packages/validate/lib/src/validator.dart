...
Map<String, dynamic> autoParse(Map inputData, Iterable<String> fields) {
  ...
  for (var key in inputData.keys) {
    if (!fields.contains(key)) {
      ...
      try {
        var n = inputData[key] is num ? inputData[key] : num.parse(inputData[key].toString());
...
class Validator extends Matcher {
  ...
  void _importSchema(Map<String, dynamic> schema) {
    for (var keys in schema.keys) {
      for (var key in keys.split(',').map((s) => s.trim())) {
        ...
        var isForbidden = _forbidden.hasMatch(key), isRequired = _asterisk.hasMatch(key);
        ...
        var _iterable = schema[keys] is Iterable ? schema[keys] : [schema[keys]];
        var iterable = [];
  ...
  static bool _hasContextValidators(Iterable it) => it.any((x) => x is ContextValidator);
  ...
  ValidationResult checkParsed(Map inputData, List<String> fields) => check(autoParse(inputData, fields));
  ...
  Map<String, dynamic> enforce(Map inputData, {String errorMessage = 'Invalid data.'}) {
  ...
  ///
  /// Otherwise, the filtered data is returned.
  Map<String, dynamic> enforceParsed(Map inputData, List<String> fields) => enforce(autoParse(inputData, fields));
  ...
  @override
  Description describe(Description description) => description.add(' passes the provided validation schema: $rules');
...
class ValidationException extends GalileoHttpException {
  /// A list of errors that resulted in the given data being marked invalid.
  ...
  ValidationException(this.message, {Iterable<String> errors = const []})
      : super(FormatException(message),
            statusCode: 400, errors: (errors ?? <String>[]).toSet().toList(), stackTrace: StackTrace.current) {
