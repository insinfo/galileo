...
class GraphQLObjectType extends GraphQLType<Map<String, dynamic>, Map<String, dynamic>>
  ...
  GraphQLInputObjectType toInputObject(String name, {String description}) {
    return new GraphQLInputObjectType(name,
        ...
        inputFields: fields
            .map((f) => new GraphQLInputObjectField(f.name, f.type.coerceToInputObject(), description: f.description)));
  }
  ...
  ValidationResult<Map<String, dynamic>> validate(String key, Map input) {
    if (input is! Map) return new ValidationResult._failure(['Expected "$key" to be a Map.']);
  ...
  Map<String, dynamic> deserialize(Map value) {
    return value.keys.fold<Map<String, dynamic>>({}, (out, k) {
      var field = fields.firstWhere((f) => f.name == k, orElse: () => null);
      if (field == null) throw new UnsupportedError('Unexpected field "$k" encountered in map.');
...
class GraphQLInputObjectType extends GraphQLType<Map<String, dynamic>, Map<String, dynamic>>
    with _NonNullableMixin<Map<String, dynamic>, Map<String, dynamic>> {
  /// The name of this type.
  final String name;
  ...
  GraphQLInputObjectType(this.name, {this.description, Iterable<GraphQLInputObjectField> inputFields: const []}) {
  ...
  ValidationResult<Map<String, dynamic>> validate(String key, Map input) {
    ...
    input.keys.forEach((k) {
      var field = inputFields.firstWhere((f) => f.name == k, orElse: () => null);
  ...
  bool operator ==(other) {
    return other is GraphQLInputObjectType &&
        other.name == name &&
        other.description == description &&
        const ListEquality<GraphQLInputObjectField>().equals(other.inputFields, inputFields);
  }
...
class GraphQLInputObjectField<Value, Serialized> {
  ...
  GraphQLInputObjectField(this.name, this.type, {this.description, this.defaultValue});
