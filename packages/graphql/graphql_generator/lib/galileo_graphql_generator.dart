...
class _GraphQLGenerator extends GeneratorForAnnotation<GraphQLClass> {
  ...
  Future<String> generateForAnnotatedElement(Element element, ConstantReader annotation, BuildStep buildStep) async {
    if (element is ClassElement) {
      var ctx = element.isEnum
          ? null
          : await buildContext(
              element, annotation, buildStep, buildStep.resolver, serializableTypeChecker.hasAnnotationOf(element));
  ...
  Expression _inferType(String className, String name, DartType type) {
    ...
    if (type is InterfaceType && _isGraphQLClass(type)) {
      var c = type;
      var name =
          serializableTypeChecker.hasAnnotationOf(c.element) && c.name.startsWith('_') ? c.name.substring(1) : c.name;
      var rc = ReCase(name);
  ...
  Library buildSchemaLibrary(ClassElement clazz, BuildContext ctx, ConstantReader ann) {
    return Library((b) {
      ...
      if (clazz.isEnum) {
        ...
        b.body.add(Field((b) {
          ...
          var named = <String, Expression>{'isInterface': literalBool(isInterface(clazz))};
          ...
          var interfaces = clazz.interfaces.where(_isGraphQLClass).map((c) {
            var name = serializableTypeChecker.hasAnnotationOf(c.element) && c.name.startsWith('_')
                ? c.name.substring(1) //c.name.substring(1)
                : c.name;
            var rc = ReCase(name);
            return refer('${rc.camelCase}GraphQLType');
          });
          ...
          //Copied from TypeDefiningElement.

          //abstract class TypeDefiningElement implements Element {
          //Return the type defined by this element.
          var search = clazz.thisType; //clazz.type is deprecated in 0.39.0 and removed in 0.40.0; use thisType
          ...
          for (var field in ctxFields) {
            var named = <String, Expression>{};
            var originalField = clazz.fields.firstWhere((f) => f.name == field.name, orElse: () => null);

            // Check if it is deprecated.
            var depEl = originalField?.getter ?? originalField ?? field;
            var depAnn = TypeChecker.fromRuntime(Deprecated).firstAnnotationOf(depEl);
            if (depAnn != null) {
            ...
            // Description finder...
            _applyDescription(named, originalField?.getter ?? originalField ?? field,
                originalField?.getter?.documentationComment ?? originalField?.documentationComment);
