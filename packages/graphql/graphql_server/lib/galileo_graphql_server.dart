...
class GraphQL {
  ...
  final FutureOr<T> Function<T>(T, String, Map<String, dynamic>) defaultFieldResolver;

  GraphQLSchema _schema;

  GraphQL(GraphQLSchema schema,
      {bool introspect = true, this.defaultFieldResolver, List<GraphQLType> customTypes = const <GraphQLType>[]})
      : _schema = schema {
      ...
      switch (ctx.typeName.name) {
        ...
        default:
          return customTypes.firstWhere((t) => t.name == ctx.typeName.name,
              orElse: () => throw ArgumentError('Unknown GraphQL type: "${ctx.typeName.name}"'));
      }
    } else {
  ...
  Future parseAndExecute(String text,
    ...
    if (parser.errors.isNotEmpty) {
      throw GraphQLException(parser.errors
          .map((e) => GraphQLExceptionError(e.message,
              locations: [GraphExceptionErrorLocation.fromSourceLocation(e.span.start)]))
          .toList());
    }
  ...
  Future executeRequest(GraphQLSchema schema, DocumentContext document,
    ...
    var coercedVariableValues = coerceVariableValues(schema, operation, variableValues ?? <String, dynamic>{});
    if (operation.isQuery) {
      return await executeQuery(document, operation, schema, coercedVariableValues, initialValue, globalVariables);
    } else if (operation.isSubscription) {
      return await subscribe(document, operation, schema, coercedVariableValues, globalVariables, initialValue);
    } else {
      return executeMutation(document, operation, schema, coercedVariableValues, initialValue, globalVariables);
    }
  }

  OperationDefinitionContext getOperation(DocumentContext document, String operationName) {
    var ops = document.definitions.whereType<OperationDefinitionContext>();

    if (operationName == null) {
      return ops.length == 1
          ? ops.first
          : throw GraphQLException.fromMessage('This document does not define any operations.');
    } else {
      return ops.firstWhere((d) => d.name == operationName,
          orElse: () => throw GraphQLException.fromMessage('Missing required operation "$operationName".'));
    }
  }

  Map<String, dynamic> coerceVariableValues(
      GraphQLSchema schema, OperationDefinitionContext operation, Map<String, dynamic> variableValues) {
    var coercedValues = <String, dynamic>{};
    var variableDefinitions = operation.variableDefinitions?.variableDefinitions ?? [];
    ...
    for (var variableDefinition in variableDefinitions) {
      ...
      if (value == null) {
        if (defaultValue != null) {
          coercedValues[variableName] = defaultValue.value.computeValue(variableValues);
        } else if (!variableType.isNullable) {
          throw GraphQLException.fromSourceSpan('Missing required variable "$variableName".', variableDefinition.span);
        }
      } else {
        ...
        if (!validation.successful) {
          throw GraphQLException(validation.errors
              .map((e) => GraphQLExceptionError(e,
                  locations: [GraphExceptionErrorLocation.fromSourceLocation(variableDefinition.span.start)]))
              .toList());
        } else {
  ...
  Future<Map<String, dynamic>> executeQuery(
    ...
    return await executeSelectionSet(document, selectionSet, queryType, initialValue, variableValues, globalVariables);
  }
  ...
  Future<Map<String, dynamic>> executeMutation(
    ...
    if (mutationType == null) {
      throw GraphQLException.fromMessage('The schema does not define a mutation type.');
    }

    var selectionSet = mutation.selectionSet;
    return await executeSelectionSet(
        document, selectionSet, mutationType, initialValue, variableValues, globalVariables);
  }
  ...
  Future<Stream<Map<String, dynamic>>> subscribe(
      ...
      Map<String, dynamic> globalVariables,
      initialValue) async {
    var sourceStream = await createSourceEventStream(document, subscription, schema, variableValues, initialValue);
    return mapSourceToResponseEvent(
        sourceStream, subscription, schema, document, initialValue, variableValues, globalVariables);
  }

  Future<Stream> createSourceEventStream(DocumentContext document, OperationDefinitionContext subscription,
      GraphQLSchema schema, Map<String, dynamic> variableValues, initialValue) {
    var selectionSet = subscription.selectionSet;
    var subscriptionType = schema.subscriptionType;
    if (subscriptionType == null) {
      throw GraphQLException.fromSourceSpan('The schema does not define a subscription type.', subscription.span);
    }
    var groupedFieldSet = collectFields(document, subscriptionType, selectionSet, variableValues);
    if (groupedFieldSet.length != 1) {
      throw GraphQLException.fromSourceSpan(
          'The grouped field set from this query must have exactly one entry.', selectionSet.span);
    }
    var fields = groupedFieldSet.entries.first.value;
    var fieldName = fields.first.field.fieldName.alias?.name ?? fields.first.field.fieldName.name;
    var field = fields.first;
    var argumentValues = coerceArgumentValues(subscriptionType, field, variableValues);
    return resolveFieldEventStream(subscriptionType, initialValue, fieldName, argumentValues);
  }
  ...
  Stream<Map<String, dynamic>> mapSourceToResponseEvent(
    ...
    await for (var event in sourceStream) {
      yield await executeSubscriptionEvent(document, subscription, schema, event, variableValues, globalVariables);
    }
  }
  ...
  Future<Map<String, dynamic>> executeSubscriptionEvent(
    ...
    try {
      var data = await executeSelectionSet(
          document, selectionSet, subscriptionType, initialValue, variableValues, globalVariables);
      return {'data': data};
  ...
  Future<Stream> resolveFieldEventStream(
      GraphQLObjectType subscriptionType, rootValue, String fieldName, Map<String, dynamic> argumentValues) async {
    var field = subscriptionType.fields.firstWhere((f) => f.name == fieldName, orElse: () {
      throw GraphQLException.fromMessage('No subscription field named "$fieldName" is defined.');
    });
  ...
  Future<Map<String, dynamic>> executeSelectionSet(
      ...
      Map<String, dynamic> variableValues,
      Map<String, dynamic> globalVariables) async {
    var groupedFieldSet = collectFields(document, objectType, selectionSet, variableValues);
    ...
    for (var responseKey in groupedFieldSet.keys) {
      ...
      for (var field in fields) {
        var fieldName = field.field.fieldName.alias?.name ?? field.field.fieldName.name;
        FutureOr<dynamic> futureResponseValue;

        if (fieldName == '__typename') {
          futureResponseValue = objectType.name;
        } else {
          var fieldType = objectType.fields.firstWhere((f) => f.name == fieldName, orElse: () => null)?.type;
          ...
          futureResponseValue = executeField(
              ...
              fields,
              fieldType,
              Map<String, dynamic>.from(globalVariables ?? <String, dynamic>{})..addAll(variableValues),
              globalVariables);
        }
  ...
  Future executeField(
    ...
    var resolvedValue = await resolveFieldValue(objectType, objectValue, fieldName,
        Map<String, dynamic>.from(globalVariables ?? {})..addAll(argumentValues ?? {}));
    return completeValue(document, fieldName, fieldType, fields, resolvedValue, variableValues, globalVariables);
  }

  Map<String, dynamic> coerceArgumentValues(
      GraphQLObjectType objectType, SelectionContext field, Map<String, dynamic> variableValues) {
    var coercedValues = <String, dynamic>{};
    var argumentValues = field.field.arguments;
    var fieldName = field.field.fieldName.alias?.name ?? field.field.fieldName.name;
    var desiredField = objectType.fields.firstWhere((f) => f.name == fieldName,
        orElse: () => throw FormatException('${objectType.name} has no field named "$fieldName".'));
    ...
    for (var argumentDefinition in argumentDefinitions) {
      ...
      var argumentValue = argumentValues.firstWhere((a) => a.name == argumentName, orElse: () => null);

      if (argumentValue == null) {
        if (defaultValue != null || argumentDefinition.defaultsToNull) {
          coercedValues[argumentName] = defaultValue;
        } else if (argumentType is GraphQLNonNullableType) {
          throw GraphQLException.fromMessage('Missing value for argument "$argumentName" of field "$fieldName".');
        } else {
          continue;
        }
      } else {
        try {
          var validation = argumentType.validate(argumentName, argumentValue.value.computeValue(variableValues));

          if (!validation.successful) {
            var errors = <GraphQLExceptionError>[
              GraphQLExceptionError(
                'Type coercion error for value of argument "$argumentName" of field "$fieldName".',
                locations: [GraphExceptionErrorLocation.fromSourceLocation(argumentValue.value.span.start)],
              )
            ];
  ...
  Future<T> resolveFieldValue<T>(
      GraphQLObjectType objectType, T objectValue, String fieldName, Map<String, dynamic> argumentValues) async {
    ...
    if (objectValue is Map) {
      ...
      if (defaultFieldResolver != null) {
        return await defaultFieldResolver(objectValue, fieldName, argumentValues);
      }
  ...
  Future completeValue(DocumentContext document, String fieldName, GraphQLType fieldType, List<SelectionContext> fields,
      result, Map<String, dynamic> variableValues, Map<String, dynamic> globalVariables) async {
    if (fieldType is GraphQLNonNullableType) {
      var innerType = fieldType.ofType;
      var completedResult =
          await completeValue(document, fieldName, innerType, fields, result, variableValues, globalVariables);

      if (completedResult == null) {
        throw GraphQLException.fromMessage('Null value provided for non-nullable field "$fieldName".');
      } else {
    ...
    if (fieldType is GraphQLListType) {
      ...
      for (var resultItem in (result as Iterable)) {
        futureOut.add(completeValue(
            document, '(item in "$fieldName")', innerType, fields, resultItem, variableValues, globalVariables));
      }
    ...
    if (fieldType is GraphQLObjectType || fieldType is GraphQLUnionType) {
      ...
      return await executeSelectionSet(document, subSelectionSet, objectType, result, variableValues, globalVariables);
    }
  ...
  GraphQLObjectType resolveAbstractType(String fieldName, GraphQLType type, result) {
    ...
    for (var t in possibleTypes) {
      try {
        var validation = t.validate(fieldName, foldToStringDynamic(result as Map));
    ...
    errors.insert(0, GraphQLExceptionError('Cannot convert value $result to type $type.'));
  ...
  Map<String, List<SelectionContext>> collectFields(DocumentContext document, GraphQLObjectType objectType,
      SelectionSetContext selectionSet, Map<String, dynamic> variableValues,
    ...
    for (var selection in selectionSet.selections) {
      ...
      if (getDirectiveValue('include', 'if', selection, variableValues) == false) {
        continue;
      }

      if (selection.field != null) {
        var responseKey = selection.field.fieldName.alias?.alias ?? selection.field.fieldName.name;
        var groupForResponseKey = groupedFields.putIfAbsent(responseKey, () => []);
        ...
        var fragment = document.definitions
            ...
            .firstWhere((f) => f.name == fragmentSpreadName, orElse: () => null);
        ...
        var fragmentGroupFieldSet = collectFields(document, objectType, fragmentSelectionSet, variableValues);
        ...
        if (fragmentType != null && !doesFragmentTypeApply(objectType, fragmentType)) continue;
  ...
  getDirectiveValue(String name, String argumentName, SelectionContext selection, Map<String, dynamic> variableValues) {
    ...
    if (vv is VariableContext) {
      ...
      if (!variableValues.containsKey(vname)) {
        throw GraphQLException.fromSourceSpan('Unknown variable: "$vname"', vv.span);
      }
  ...
  bool doesFragmentTypeApply(GraphQLObjectType objectType, TypeConditionContext fragmentType) {
