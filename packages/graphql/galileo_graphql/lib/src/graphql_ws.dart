...
import 'package:galileo_graphql_server/subscriptions_transport_ws.dart' as stw;
  ...
  return (req, res) async {
    if (req is HttpRequestContext) {
      if (WebSocketTransformer.isUpgradeRequest(req.rawRequest)) {
        await res.detach();
        var socket = await WebSocketTransformer.upgrade(req.rawRequest, protocolSelector: (protocols) {
          if (protocols.contains('graphql-ws')) {
            return 'graphql-ws';
          } else {
            throw GalileoHttpException.badRequest(message: 'Only the "graphql-ws" protocol is allowed.');
          }
        });
        var channel = IOWebSocketChannel(socket);
        var client = stw.RemoteClient(channel.cast<String>());
        var server = _GraphQLWSServer(client, graphQL, req, res, keepAliveInterval);
        await server.done;
      } else {
        throw GalileoHttpException.badRequest(message: 'The `graphQLWS` endpoint only accepts WebSockets.');
      }
    } else {
      throw GalileoHttpException.badRequest(message: 'The `graphQLWS` endpoint only accepts HTTP/1.1 requests.');
    }
  };
}
...
class _GraphQLWSServer extends stw.Server {
  ...
  _GraphQLWSServer(stw.RemoteClient client, this.graphQL, this.req, this.res, Duration keepAliveInterval)
