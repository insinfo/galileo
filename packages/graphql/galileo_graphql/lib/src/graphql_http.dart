...
RequestHandler graphQLHttp(GraphQL graphQL,
    {Function(RequestContext, ResponseContext, Stream<Map<String, dynamic>>) onSubscription}) {
    ...
    sendGraphQLResponse(result) async {
      if (result is Stream<Map<String, dynamic>>) {
        if (onSubscription == null) {
          throw StateError('The GraphQL backend returned a Stream, but no `onSubscription` callback was provided.');
        } else {
    ...
    try {
      if (req.method == 'GET') {
        ...
        if (req.headers.contentType?.mimeType == graphQlContentType.mimeType) {
          ...
          if (operations == null) {
            throw GalileoHttpException.badRequest(message: 'Missing "operations" field.');
          }
          var map = fields.containsKey('map') ? json.decode(fields['map'] as String) : null;
          if (map is! Map) {
            throw GalileoHttpException.badRequest(message: '"map" field must decode to a JSON object.');
          }
          var variables = Map<String, dynamic>.from(globalVariables);
          for (var entry in (map as Map).entries) {
            var file = req.uploadedFiles.firstWhere((f) => f.name == entry.key, orElse: () => null);
            if (file == null) {
              throw GalileoHttpException.badRequest(
                  message: '"map" contained key "${entry.key}", but no uploaded file '
                      'has that name.');
            }
            if (entry.value is! List) {
              throw GalileoHttpException.badRequest(
                  message: 'The value for "${entry.key}" in the "map" field was not a JSON array.');
            }
              ...
              if (subPaths[0] == 'variables') {
                ...
                for (int i = 1; i < subPaths.length; i++) {
                  ...
                  if (_num.hasMatch(name)) {
                    if (current is! List) {
                      throw GalileoHttpException.badRequest(
                          message: 'Object "$parent" is not a JSON array, but the '
                    ...
                    if (current is! Map) {
                      throw GalileoHttpException.badRequest(
                          message: 'Object "$parent" is not a JSON object, but the '
                              '"map" field contained a mapping to $parent.$name.');
                    }
      ...
      if (req.app?.logger != null) {
        req.app.logger.severe('An error occurred while processing GraphQL query at ${req.uri}.', e, st);
      }
